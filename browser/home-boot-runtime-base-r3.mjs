...
const RANK_MATCH_TEXTURES = Object.freeze({
  waitingBackground: './assets/visual/rank-match/waiting-bg.webp',
  uiChrome: './assets/visual/rank-match/ui-chrome.webp',
  partnerPicker: './assets/visual/rank-match/partner-picker.webp',
  partnerPickerModal: './assets/visual/rank-match/picker-modal.webp',
  partnerPickerTile: './assets/visual/rank-match/picker-tile-selected.webp',
  buttonPrimary: './assets/visual/rank-match/button-primary.webp',
  buttonSecondary: './assets/visual/rank-match/button-secondary.webp',
  actionRing: './assets/visual/rank-match/action-ring.webp',
  waitingVfx: './assets/visual/rank-match/waiting-vfx.webp',
});
const RANK_MATCH_SETUP_STYLE_ID = 'gameroad-rank-match-flow-r1-style';
const RANK_MATCH_PICKER_ID = 'gameroad-rank-partner-picker';
const RANK_MATCH_WAITING_ID = 'gameroad-rank-match-waiting-surface';
const RANK_MATCH_MODE_SECTION = 'data-rank-match-controls';
const RANK_MATCH_PICKER_SCHEMA = 'gameroad.rank-match-partner-picker.v1';
const RANK_MATCH_WAITING_SCHEMA = 'gameroad.rank-match-waiting-presentation.v1';
...
const rankMatchRuntime = {
  ...
  selectedMode: 'normal',
  waiting: false,
  waitingPartnerId: 'partner.saasuna',
  picker: null,
  lastFocus: null,
};

function rankMatchDocument() {
  return typeof document !== 'undefined' ? document : null;
}

function rankMatchWindow() {
  return typeof window !== 'undefined' ? window : globalThis;
}

function rankMatchElement(value) {
  return Boolean(value && value.nodeType === 1);
}

function rankMatchExactId(value) {
  if (typeof value !== 'string') return null;
  const token = value.trim();
  return token && token === value && token.length <= 160 ? token : null;
}

function ensureRankMatchStyle(doc) {
  if (!doc || doc.getElementById(RANK_MATCH_SETUP_STYLE_ID)) return;
  const style = doc.createElement('style');
  style.id = RANK_MATCH_SETUP_STYLE_ID;
  style.textContent = [
    ':root{--rank-match-waiting-bg:url("', RANK_MATCH_TEXTURES.waitingBackground, '");',
    '--rank-match-ui-chrome:url("', RANK_MATCH_TEXTURES.uiChrome, '");',
    '--rank-match-picker:url("', RANK_MATCH_TEXTURES.partnerPicker, '");',
    '--rank-match-picker-modal:url("', RANK_MATCH_TEXTURES.partnerPickerModal, '");',
    '--rank-match-picker-tile:url("', RANK_MATCH_TEXTURES.partnerPickerTile, '");',
    '--rank-match-button-primary:url("', RANK_MATCH_TEXTURES.buttonPrimary, '");',
    ...
    '.gameroadRankMatchMode:hover,.gameroadRankMatchMode:focus-visible{filter:brightness(1.16);border-color:#a0efd5;outline:2px solid rgba(160,239,213,.24);outline-offset:2px;}','.gameroadRankMatchMode:active{transform:translateY(1px) scale(.985);filter:brightness(1.08);}',
    '.gameroadRankMatchMode[aria-pressed="true"]{border-color:#ffd27e;background:linear-gradient(135deg,rgba(122,77,18,.96),rgba(45,39,19,.88));box-shadow:0 0 0 1px rgba(255,210,126,.22) inset,0 0 18px rgba(255,210,126,.16);}',
    '.gameroadRankMatchMode[data-rank-mode="rank"][aria-pressed="true"]{background:linear-gradient(135deg,rgba(26,103,82,.96),rgba(37,60,50,.92));border-color:#a0efd5;box-shadow:0 0 0 1px rgba(160,239,213,.28) inset,0 0 18px rgba(160,239,213,.18);}',
    '.gameroadRankMatchMode small{display:block;margin-top:4px;color:#a6c6bc;font-size:9px;font-weight:700;}',
    '.gameroadRankMatchSetup .rankMatchModeNote{font-size:9px;color:#a6c6bc;line-height:1.45;}',
    'section[data-screen="battle"][data-rank-waiting="true"] #battleRuntime,section[data-screen="battle"][data-rank-waiting="true"] .battleAvatar,section[data-screen="battle"][data-rank-waiting="true"] #partnerAdviceChatPresentation,section[data-screen="battle"][data-rank-waiting="true"] [data-role="advice-partner-bustup"]{visibility:hidden!important;pointer-events:none!important;}',
    'section[data-screen="battle"][data-rank-waiting="true"] #battleMap,section[data-screen="battle"][data-rank-waiting="true"] .battleSide{filter:brightness(.5) saturate(.55);}',
    '.gameroadRankWaiting{position:absolute;inset:0;z-index:90;overflow:hidden;background:#071714;color:#f5fff9;isolation:isolate;}',
    '.gameroadRankWaiting[hidden]{display:none!important;}',
    '.rankWaitingBackdrop{position:absolute;inset:0;background-image:var(--rank-match-waiting-bg);background-size:cover;background-position:center center;filter:saturate(.86) brightness(.72);transform:scale(1.04);}',
    '.rankWaitingBackdrop:after{content:"";position:absolute;inset:0;background:linear-gradient(90deg,rgba(4,17,14,.12),rgba(4,17,14,.62) 58%,rgba(3,13,12,.9));}',
    '.rankWaitingVfx{position:absolute;inset:0;background-image:var(--rank-match-waiting-vfx);background-size:400% 100%;background-position:0 52%;background-repeat:no-repeat;mix-blend-mode:screen;opacity:.36;pointer-events:none;animation:gameroadRankWaitingVfx 3.2s steps(4,end) infinite;}',
    '@keyframes gameroaddRankWaitingVfx{from{background-position:0 52%;}to{background-position:100% 52%;}}',
    '@keyframes gameroaddRankWaitingVfx{from{background-position:0 52%;}to{background-position:100% 52%;}}',
    '.rankWaitingPanel{position:absolute;left:clamp(12px,5vw,82px);right:clamp(12px,5vw,82px);top:clamp(12px,8vh,72px);bottom:clamp(12px,8vh,72px);display:grid;grid-template-rows:auto 1fr auto;overflow:hidden;border:1px solid rgba(255,210,126,.74);background:linear-gradient(135deg,rgba(29,44,35,.86),rgba(42,28,12,.72)),var(--rank-match-ui-chrome);background-size:cover,cover;background-position:center,center;box-shadow:0 18px 60px rgba(0,0,0,.52),0 0 0 1px rgba(255,255,255,.08) inset;clip-path:polygon(1.2% 0,98.8% 0,100% 2%,100% 98%,98.8% 100%,1.2% 100%,0 98%,0 2%);animation:gameroadRankWaitingPanelIn 360ms cubic-bezier(.16,1,.3,1) both;}',
    '@keyframes gameroadRankWaitingPanelIn{0%{opacity:0;transform:translateY(12px) scale(.985)}100%{opacity:1;transform:none}}',
    '@keyframes gameroadRankWaitingStageIn{0%{opacity:0;transform:translateY(10px)}100%{opacity:1;transform:none}}',
    '.rankWaitingPanel:before{content:"";position:absolute;inset:8px;border:1px solid rgba(255,233,165,.26);pointer-events:none;clip-path:inherit;}',
    '.rankWaitingHeader{position:relative;z-index:2;display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:10px;padding:12px clamp(12px,2vw,28px);border-bottom:1px solid rgba(255,210,126,.32);background:linear-gradient(180deg,rgba(3,18,15,.66),rgba(3,18,15,.12));}',
    '.rankWaitingBack,.rankWaitingCancel{min-height:44px;border:1px solid rgba(197,246,228,.34);background:linear-gradient(135deg,rgba(18,66,55,.88),rgba(6,25,21,.88));color:#f5fff9;padding:0 14px;font-weight:900;cursor:pointer;transition:transform 90ms ease,filter 90ms ease,border-color 120ms ease;}',
    '.rankWaitingBack:hover,.rankWaitingBack:focus-visible,.rankWaitingCancel:hover,.rankWaitingCancel:focus-visible{border-color:#a0efd5;filter:brightness(1.14);outline:2px solid rgba(160,239,213,.2);outline-offset:2px;}','.rankWaitingBack:active,.rankWaitingCancel:active{transform:translateY(1px) scale(.985);filter:brightness(1.08);}',
    '.rankWaitingTitle{text-align:center;line-height:1.1;}.rankWaitingTitle small{display:block;color:#a6c6bc;font-size:9px;letter-spacing:.16em;}.rankWaitingTitle strong{display:block;color:#fff0b5;font-size:clamp(17px,2.2vw,30px);letter-spacing:.08em;text-shadow:0 2px 10px rgba(0,0,0,.45);}',
    '.rankWaitingContent{position:relative;z-index:2;display:grid;grid-template-columns:minmax(150px,.8fr) minmax(250px,1.35fr) minmax(150px,.8fr);align-items:center;gap:clamp(14px,3vw,56px);padding:clamp(18px,4vh,44px) clamp(20px,4vw,70px);min-height:0;}',
    '.rankWaitingPartnerRail{display:grid;justify-items:center;gap:10px;min-width:0;animation:gameroadRankWaitingStageIn 300ms cubic-bezier(.16,1,.3,1) 90ms both;}.rankWaitingPartnerLabel{font-size:10px;color:#c9e6d9;letter-spacing:.08em;text-align:center;}.rankWaitingPartner{position:relative;width:min(18vw,220px);min-width:128px;aspect-ratio:1/1.18;border:1px solid rgba(255,224,143,.66);border-radius:18px;background:radial-gradient(circle at 50% 70%,rgba(51,194,157,.24),transparent 58%),linear-gradient(145deg,rgba(9,51,42,.88),rgba(18,34,29,.9)),var(--rank-match-action-ring);background-size:auto,auto,cover;background-position:center;box-shadow:0 0 0 2px rgba(255,255,255,.05) inset,0 12px 34px rgba(0,0,0,.38);overflow:hidden;cursor:pointer;transition:filter 160ms ease,transform 160ms ease,border-color 160ms ease;display:grid;place-items:end center;padding:8px;}',
    '.rankWaitingPartner:hover,.rankWaitingPartner:focus-visible{transform:translateY(-3px) scale(1.02);filter:brightness(1.14);border-color:#a0efd5;outline:3px solid rgba(160,239,213,.24);outline-offset:3px;}','.rankWaitingPartner:active{transform:translateY(1px) scale(.985);filter:brightness(1.08);}',
    '.rankWaitingPartner img{display:block;width:100%;height:100%;object-fit:contain;object-position:center bottom;filter:drop-shadow(0 8px 12px rgba(0,0,0,.45));}.rankWaitingPartner .rankWaitingPartnerIcon{width:70%;aspect-ratio:1;display:grid;place-items:center;border-radius:50%;border:2px solid rgba(255,224,143,.68);background:rgba(4,20,16,.66);color:#fff0b5;font-size:clamp(30px,5vw,70px);font-weight:1000;box-shadow:0 0 20px rgba(160,239,213,.24);}',
    '.rankWaitingPartnerName{font-weight:1000;color:#fff0b5;text-align:center;}.rankWaitingPartnerHint{font-size:9px;color:#a6c6bc;text-align:center;}',
    '.rankWaitingCopy{min-width:0;text-align:center;display:grid;justify-items:center;gap:11px;animation:gameroadRankWaitingStageIn 300ms cubic-bezier(.16,1,.3,1) 150ms both;}.rankWaitingEyebrow{font-size:10px;letter-spacing:.2em;color:#a0efd5;font-weight:1000;}.rankWaitingCopy h1{margin:0;color:#fff3ba;font-size:clamp(32px,5vw,72px);line-height:.96;letter-spacing:.08em;text-shadow:0 5px 18px rgba(0,0,0,.5);}.rankWaitingCopy p{max-width:34em;margin:0;color:#d7ece2;font-size:clamp(11px,1.2vw,15px);line-height:1.7;}.rankWaitingStatus{display:inline-flex;align-items:center;gap:8px;min-height:42px;padding:0 17px;border:1px solid rgba(160,239,213,.44);background:rgba(4,24,20,.72);color:#dff7ef;font-weight:900;box-shadow:0 0 0 1px rgba(160,239,213,.08) inset;}.rankWaitingStatus:before{content:"";width:9px;height:9px;border-radius:50%;background:#ffd27e;box-shadow:0 0 12px #ffd27e;animation:gameroadRankWaitingPulse 1.8s ease-in-out infinite;}',
    '@keyframes gameroaddRankWaitingPulse{0%,100%{opacity:.42;transform:scale(.8)}50%{opacity:1;transform:scale(1.15)}}',
    '.rankWaitingMeta{font-size:9px;color:#a6c6bc;letter-spacing:.06em;}.rankWaitingActions{display:grid;gap:10px;align-content:center;justify-items:stretch;animation:gameroadRankWaitingStageIn 300ms cubic-bezier(.16,1,.3,1) 210ms both;}.rankWaitingAction{min-height:52px;min-width:150px;padding:0 18px;border:1px solid rgba(255,226,149,.72);background-image:linear-gradient(rgba(16,54,44,.52),rgba(16,54,44,.52)),var(--rank-match-button-primary);background-size:cover;color:#fff4bf;font-weight:1000;letter-spacing:.06em;text-shadow:0 2px 5px rgba(0,0,0,.6);cursor:pointer;transition:transform 90ms ease,filter 90ms ease,border-color 120ms ease;}.rankWaitingAction.secondary{background-image:linear-gradient(rgba(9,29,24,.52),rgba(9,29,24,.52)),var(--rank-match-button-secondary);border-color:rgba(197,246,228,.46);}.rankWaitingAction:hover,.rankWaitingAction:focus-visible{filter:brightness(1.16);transform:translateY(-1px);outline:2px solid rgba(255,232,156,.3);outline-offset:3px;}.rankWaitingAction:active{transform:translateY(1px) scale(.985);filter:brightness(1.08);}.rankWaitingFoot{position:relative;z-index:2;display:flex;align-items:center;justify-content:center;min-height:42px;padding:8px 16px;border-top:1px solid rgba(255,210,126,.24);color:#a6c6bc;font-size:9px;letter-spacing:.05em;text-align:center;}',
    '.gameroadRankPartnerPicker{position:fixed;inset:0;z-index:150;display:grid;place-items:center;padding:clamp(10px,3vw,34px);background:rgba(2,10,8,.74);backdrop-filter:blur(6px);}.gameroadRankPartnerPicker[hidden]{display:none!important;}.rankPartnerPickerShell{position:relative;width:min(86vmin,680px);aspect-ratio:1/1;max-width:92vw;max-height:90vh;overflow:auto;padding:clamp(18px,3vw,34px);border:1px solid rgba(255,224,143,.78);background:linear-gradient(rgba(12,42,35,.76),rgba(12,42,35,.76)),var(--rank-match-picker-modal);background-size:cover;background-position:center;box-shadow:0 20px 70px rgba(0,0,0,.62),0 0 0 1px rgba(255,255,255,.08) inset;clip-path:polygon(2% 0,98% 0,100% 2%,100% 98%,98% 100%,2% 100%,0 98%,0 2%);}.rankPartnerPickerShell:before{content:"";position:absolute;inset:10px;border:1px solid rgba(255,237,177,.22);pointer-events:none;}.rankPartnerPickerShell:after{content:"";position:absolute;z-index:0;inset:0;background-image:var(--rank-match-picker);background-size:cover;background-position:center;mix-blend-mode:screen;opacity:.28;pointer-events:none;}.rankPartnerPickerHeader{position:relative;z-index:1;display:flex;align-items:center;justify-content:space-between;gap:14px;padding-bottom:14px;border-bottom:1px solid rgba(255,224,143,.3);}.rankPartnerPickerHeader strong{color:#fff0b5;font-size:clamp(18px,3vw,30px);letter-spacing:.08em;}.rankPartnerPickerHeader small{display:block;color:#a6c6bc;font-size:9px;margin-top:4px;}.rankPartnerPickerClose{width:44px;height:44px;border:1px solid rgba(197,246,228,.42);background:rgba(6,26,21,.82);color:#fff4bf;font-size:22px;cursor:pointer;transition:transform 90ms ease,filter 90ms ease,border-color 120ms ease;}.rankPartnerPickerClose:hover,.rankPartnerPickerClose:focus-visible{border-color:#a0efd5;outline:2px solid rgba(160,239,213,.25);outline-offset:2px;}.rankPartnerPickerClose:active{transform:translateY(1px) scale(.94);filter:brightness(1.08);}.rankPartnerChoiceGrid{position:relative;z-index:1;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-top:16px;}.rankPartnerChoice{position:relative;display:grid;grid-template-columns:minmax(76px,100px) 1fr;align-items:center;gap:12px;min-height:112px;padding:9px;border:1px solid rgba(197,246,228,.32);background:linear-gradient(rgba(8,31,25,.74),rgba(8,31,25,.74)),var(--rank-match-picker-tile);background-size:cover;color:#f5fff9;text-align:left;cursor:pointer;transition:transform 90ms ease,filter 90ms ease,border-color 120ms ease;}.rankPartnerChoice[aria-pressed="true"]{border-color:#ffd27e;box-shadow:0 0 0 2px rgba(255,210,126,.18) inset,0 0 18px rgba(255,210,126,.18);}.rankPartnerChoice:hover,.rankPartnerChoice:focus-visible{filter:brightness(1.15);outline:2px solid rgba(160,239,213,.28);outline-offset:2px;}.rankPartnerChoice:active{transform:scale(.985);filter:brightness(1.08);}.rankPartnerChoiceIcon{display:grid;place-items:center;aspect-ratio:1;border-radius:50%;overflow:hidden;border:2px solid rgba(255,224,143,.68);background:radial-gradient(circle,rgba(97,208,173,.26),rgba(5,20,16,.84));color:#fff0b5;font-size:33px;font-weight:1000;}.rankPartnerChoiceIcon img{width:100%;height:100%;object-fit:contain;}.rankPartnerChoiceText strong{display:block;color:#fff0b5;font-size:13px;}.rankPartnerChoiceText small{display:block;margin-top:6px;color:#a6c6bc;font-size:9px;line-height:1.4;}.rankPartnerPickerHint{position:relative;z-index:1;margin:14px 0 0;color:#a6c6bc;font-size:9px;line-height:1.5;text-align:center;}',
    '@media(max-width:780px){.rankWaitingContent{grid-template-columns:minmax(112px,.65fr) minmax(210px,1.35fr);gap:14px;padding:22px 20px;}.rankWaitingActions{grid-column:1 / -1;grid-template-columns:1fr 1fr;justify-items:stretch;}.rankWaitingPartner{width:min(30vw,190px);min-width:108px;}.rankPartnerChoiceGrid{grid-template-columns:1fr;}}',
    '@media(max-height:430px) and (orientation:landscape){.rankWaitingPanel{left:8px;right:8px;top:6px;bottom:6px;}.rankWaitingHeader{grid-template-columns:auto 1fr auto;padding:6px 10px;}.rankWaitingBack,.rankWaitingCancel{min-height:44px;padding:0 10px;}.rankWaitingTitle small{font-size:7px;}.rankWaitingTitle strong{font-size:18px;}.rankWaitingContent{grid-template-columns:minmax(96px,.55fr) minmax(190px,1.35fr) minmax(132px,.72fr);gap:10px;padding:8px 12px;overflow:hidden;}.rankWaitingPartnerRail{gap:4px;}.rankWaitingPartnerLabel,.rankWaitingPartnerHint,.rankWaitingMeta{font-size:7px;}.rankWaitingPartner{width:min(17vw,112px);min-width:88px;aspect-ratio:1/1.08;border-radius:12px;padding:4px;}.rankWaitingCopy{gap:5px;}.rankWaitingCopy h1{font-size:clamp(24px,5.2vw,38px);}.rankWaitingCopy p{font-size:9px;line-height:1.35;max-width:29em;}.rankWaitingStatus{min-height:32px;padding:0 10px;font-size:9px;}.rankWaitingActions{grid-column:auto;grid-template-columns:1fr;gap:6px;}.rankWaitingAction{min-width:118px;min-height:44px;padding:0 10px;font-size:10px;}.rankWaitingFoot{min-height:28px;padding:4px 10px;font-size:7px;}.rankPartnerPickerShell{width:min(92vh,92vw,680px);max-height:92vh;padding:12px;}.rankPartnerPickerHeader{padding-bottom:8px;}.rankPartnerChoiceGrid{grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin-top:8px;}.rankPartnerChoice{grid-template-columns:58px 1fr;min-height:76px;padding:6px;gap:8px;}.rankPartnerPickerHint{margin-top:8px;font-size:7px;}}',
    '@media(max-width:540px) and (orientation:portrait){.rankWaitingPanel{left:7px;right:7px;top:7px;bottom:7px;}.rankWaitingHeader{grid-template-columns:auto 1fr;}.rankWaitingCancel{grid-column:2;justify-self:end;grid-row:1;}.rankWaitingTitle{grid-column:1 / -1;grid-row:2;}.rankWaitingContent{grid-template-columns:1fr;align-content:center;gap:14px;padding:16px 14px;overflow:auto;}.rankWaitingPartnerRail{order:0;}.rankWaitingPartner{width:min(42vw,170px);min-width:118px;}.rankWaitingCopy{order:1;}.rankWaitingActions{order:2;grid-column:auto;grid-template-columns:1fr;}.rankWaitingCopy h1{font-size:clamp(30px,10vw,50px);}.rankWaitingFoot{min-height:36px;font-size:8px;}.rankPartnerChoice{grid-template-columns:82px 1fr;min-height:96px;}}',
    '@media(prefers-reduced-motion:reduce){.rankWaitingVfx,.rankWaitingStatus:before,.rankWaitingPanel,.rankWaitingPartnerRail,.rankWaitingCopy,.rankWaitingActions{animation:none!important;}.gameroadRankMatchMode,.rankWaitingBack,.rankWaitingCancel,.rankWaitingPartner,.rankWaitingAction,.rankPartnerPickerClose,.rankPartnerChoice{transition:none!important;}.gameroadRankMatchMode:active,.rankWaitingBack:active,.rankWaitingCancel:active,.rankWaitingPartner:hover,.rankWaitingPartner:focus-visible,.rankWaitingPartner:active,.rankWaitingAction:hover,.rankWaitingAction:focus-visible,.rankWaitingAction:active,.rankPartnerPickerClose:active,.rankPartnerChoice:active{transform:none!important;}}',
  ].join('');
  style.textContent = style.textContent.replaceAll('gameroadd', 'gameroad');
  doc.head.append(style);
}

function rankMatchActiveSetup(doc) {
  return doc?.querySelector?.('section[data-screen="setup"].active') || null;
}
...
function setRankMatchSetupMode(section, mode) {
  const next = mode === 'rank' ? 'rank' : 'normal';
  rankMatchRuntime.selectedMode = next;
  if (section?.getAttribute?.('data-rank-match-mode') !== next) section.setAttribute('data-rank-match-mode', next);
  for (const button of section?.querySelectorAll?.('[data-rank-mode]') || []) {
    const pressed = button.dataset.rankMode === next ? 'true' : 'false';
    if (button.getAttribute('aria-pressed') !== pressed) button.setAttribute('aria-pressed', pressed);
  }
  const note = section?.querySelector?.('.rankMatchModeNote');
  if (note) {
    const nextNote = next === 'rank'
      ? 'バトル後のランクマッチ待機画面へ進みます。'
      : '通常のローカル対戦設定をそのまま使用します。';
    if (note.textContent !== nextNote) note.textContent = nextNote;
  }
  const start = rankMatchSetupStart(section);
  if (start) {
    if (!start.dataset.rankDefaultLabel) start.dataset.rankDefaultLabel = start.textContent || '盤面へ';
    const nextLabel = next === 'rank' ? 'ランクマッチ待機へ' : start.dataset.rankDefaultLabel;
    if (start.textContent !== nextLabel) start.textContent = nextLabel;
    start.dataset.rankMatchMode = next;
  }
}

function ensureRankMatchSetupControls(doc) {
  const setup = rankMatchActiveSetup(doc);
  if (!setup) return null;
  const box = setup.querySelector('.setupBox');
  if (!box) return null;
  let section = box.querySelector('[' + RANK_MATCH_MODE_SECTION + '="true"]');
  if (!section) {
    section = doc.createElement('section');
    section.className = 'gameroadRankMatchSetup';
    section.setAttribute(RANK_MATCH_MODE_SECTION, 'true');
    const title = doc.createElement('div');
    title.className = 'k';
    title.textContent = 'MATCH TYPE';
    const row = doc.createElement('div');
    row.className = 'gameroadRankMatchModeRow';
    const normal = doc.createElement('button');
    normal.type = 'button';
    normal.className = 'gameroadRankMatchMode';
    normal.dataset.rankMode = 'normal';
    normal.textContent = '通常マッチ';
    const normalHint = doc.createElement('small');
    normalHint.textContent = '現在の設定で開始';
    normal.append(normalHint);
    const ranked = doc.createElement('button');
    ranked.type = 'button';
    ranked.className = 'gameroadRankMatchMode';
    ranked.dataset.rankMode = 'rank';
    ranked.textContent = 'ランクマッチ';
    const rankedHint = doc.createElement('small');
    rankedHint.textContent = 'バトル後に待機';
    ranked.append(rankedHint);
    row.append(normal, ranked);
    const note = doc.createElement('div');
    note.className = 'rankMatchModeNote';
    section.append(title, row, note);
    const start = rankMatchSetupStart(setup);
    if (start) box.insertBefore(section, start);
    else box.append(section);
    row.addEventListener('click', (event) => {
      const button = event.target?.closest?.('[data-rank-mode]');
      if (!button || !row.contains(button)) return;
      setRankMatchSetupMode(section, button.dataset.rankMode);
    });
  }
  setRankMatchSetupMode(section, rankMatchRuntime.selectedMode);
  return section;
}

function rankMatchPartnerIds(win) {
  let ids = [];
  try {
    ids = partnerRosterIdsFromRuntime(win).filter((id) => rankMatchExactId(id));
  } catch {}
  return ids.length ? ids : ['partner.saasuna'];
}

function rankMatchPartnerName(id) {
  try {
    return partnerDisplayName(id);
  } catch {
    return id === 'partner.saasuna' ? 'サースナー' : 'パートナー';
  }
}

function rankMatchPartnerInitial(id) {
  const name = rankMatchPartnerName(id);
  return Array.from(name).filter((char) => char.trim()).slice(0, 1).join('') || 'P';
}

function rankMatchPartnerAsset(id) {
  return id === 'partner.saasuna' ? SAASUNA_BUSTUP_ASSETS.IDLE_GENTLE?.src || null : null;
}

function rankMatchRenderPartner(surface, doc) {
  const id = rankMatchRuntime.waitingPartnerId || 'partner.saasuna';
  const name = rankMatchPartnerName(id);
  const image = surface.querySelector('[data-role="rank-waiting-partner-image"]');
  const icon = surface.querySelector('[data-role="rank-waiting-partner-icon"]');
...
function rankMatchCreatePicker(doc) {
  let overlay = doc.getElementById(RANK_MATCH_PICKER_ID);
  if (overlay) return overlay;
  overlay = doc.createElement('div');
  overlay.id = RANK_MATCH_PICKER_ID;
  overlay.className = 'gameroadRankPartnerPicker';
  overlay.hidden = true;
  overlay.setAttribute('role', 'dialog');
  overlay.setAttribute('aria-modal', 'true');
  overlay.setAttribute('aria-labelledby', 'rankPartnerPickerTitle');
  overlay.dataset.schema = RANK_MATCH_PICKER_SCHEMA;
  overlay.innerHTML = '<div class="rankPartnerPickerShell"><div class="rankPartnerPickerHeader"><div><strong id="rankPartnerPickerTitle">アドバイスパートナー</strong><small>待機画面に表示するバストアップを選択</small></div><button type="button" class="rankPartnerPickerClose" data-rank-picker-close aria-label="キャラ選択を閉じる">×</button></div><div class="rankPartnerChoiceGrid" data-rank-partner-choice-grid></div><p class="rankPartnerPickerHint">操作キャラの設定は変更せず、待機画面のアドバイス役だけを切り替えます。</p></div>';
  doc.body.append(overlay);
  overlay.addEventListener('click', (event) => {
    if (event.target === overlay || event.target?.closest?.('[data-rank-picker-close]')) {
      closeRankMatchPartnerPicker(doc);
      return;
    }
    const button = event.target?.closest?.('[data-partner-choice]');
    if (!button) return;
    const id = rankMatchExactId(button.dataset.partnerChoice);
    if (!id) return;
    rankMatchRuntime.waitingPartnerId = id;
    try { setAdvicePartnerId(rankMatchWindow(), id); } catch {}
    rankMatchRenderPartner(doc.getElementById(RANK_MATCH_WAITING_ID), doc);
    closeRankMatchPartnerPicker(doc);
  });
  return overlay;
}

function rankMatchRenderPicker(doc) {
  const overlay = rankMatchCreatePicker(doc);
  const grid = overlay.querySelector('[data-rank-partner-choice-grid]');
  if (!grid) return overlay;
  const current = rankMatchRuntime.waitingPartnerId || 'partner.saasuna';
  grid.replaceChildren();
  for (const id of rankMatchPartnerIds(rankMatchWindow())) {
    const choice = doc.createElement('button');
    choice.type = 'button';
    choice.className = 'rankPartnerChoice';
    choice.dataset.partnerChoice = id;
    choice.setAttribute('aria-pressed', id === current ? 'true' : 'false');
    const icon = doc.createElement('span');
    icon.className = 'rankPartnerChoiceIcon';
    const asset = rankMatchPartnerAsset(id);
    if (asset) {
      ...
      icon.textContent = rankMatchPartnerInitial(id);
    }
    const text = doc.createElement('span');
    text.className = 'rankPartnerChoiceText';
    const name = doc.createElement('strong');
    name.textContent = rankMatchPartnerName(id);
    const hint = doc.createElement('small');
    hint.textContent = id === 'partner.saasuna' ? '正規バストアップ' : 'アドバイス役';
    text.append(name, hint);
    choice.append(icon, text);
    grid.append(choice);
  }
  return overlay;
}
...
function rankMatchBuildWaitingSurface(doc) {
  let surface = doc.getElementById(RANK_MATCH_WAITING_ID);
  if (surface) return surface;
  surface = doc.createElement('section');
  surface.id = RANK_MATCH_WAITING_ID;
  surface.className = 'gameroadRankWaiting';
  surface.hidden = true;
  surface.dataset.schema = RANK_MATCH_WAITING_SCHEMA;
  surface.setAttribute('aria-label', 'ランクマッチ待機画面');
  surface.innerHTML = '<div class="rankWaitingBackdrop" aria-hidden="true"></div><div class="rankWaitingVfx" aria-hidden="true"></div><div class="rankWaitingPanel"><header class="rankWaitingHeader"><button type="button" class="rankWaitingBack" data-rank-waiting-back>‹ バトル設定</button><div class="rankWaitingTitle"><small>RANKED MATCH / WAITING ROOM</small><strong>ランクマッチ</strong></div><button type="button" class="rankWaitingCancel" data-rank-waiting-cancel>キャンセル</button></header><div class="rankWaitingContent"><div class="rankWaitingPartnerRail"><div class="rankWaitingPartnerLabel">アドバイスパートナー</div><button type="button" class="rankWaitingPartner" data-rank-waiting-partner aria-label="アドバイスパートナーを変更"><img data-role="rank-waiting-partner-image" alt="" draggable="false"><span class="rankWaitingPartnerIcon" data-role="rank-waiting-partner-icon" aria-hidden="true"></span></button><strong class="rankWaitingPartnerName" data-role="rank-waiting-partner-name">サースナー</strong><span class="rankWaitingPartnerHint">タップして変更</span></div><div class="rankWaitingCopy"><div class="rankWaitingEyebrow">MATCHMAKING READY</div><h1>ランクマッチ</h1><p>バトル設定を引き継いだまま、ランクマッチの待機画面へ移動しました。</p><div class="rankWaitingStatus" data-role="rank-provider-status" aria-live="polite">接続先を待機中</div><div class="rankWaitingMeta">操作キャラ：設定済み　／　アドバイス役：選択可能</div></div><div class="rankWaitingActions"><button type="button" class="rankWaitingAction" data-rank-waiting-primary>接続先を確認</button><button type="button" class="rankWaitingAction secondary" data-rank-waiting-cancel>待機をやめる</button></div></div><footer class="rankWaitingFoot">ランクマッチの接続先が登録されると、この待機面から正式な検索状態へ接続します。</footer></div>';
  doc.body.append(surface);
  surface.addEventListener('click', (event) => {
    if (event.target?.closest?.('[data-rank-waiting-partner]')) {
      openRankMatchPartnerPicker(doc);
      return;
    }
    if (event.target?.closest?.('[data-rank-waiting-back],[data-rank-waiting-cancel]')) {
      exitRankMatchWaiting(doc);
      return;
    }
    if (event.target?.closest?.('[data-rank-waiting-primary]')) {
      const status = surface.querySelector('[data-role="rank-provider-status"]');
      const provider = rankMatchWindow().GAMEROAD_RANK_MATCH_SEARCH;
      if (typeof provider === 'function') {
        try {
          const result = provider({ mode: 'rank', surface: 'waiting', presentationOnly: true });
          if (result?.status) status.textContent = String(result.status);
          else status.textContent = '接続先の状態を確認中';
        } catch {
          status.textContent = '接続先の状態を取得できません';
        }
      } else {
        status.textContent = '接続先は未登録です';
      }
    }
  });
  return surface;
...
function rankMatchSyncBattleVisibility(doc) {
  const battle = doc.querySelector('section[data-screen="battle"]');
  const surface = rankMatchBuildWaitingSurface(doc);
  if (!battle || !surface) return null;
  const active = rankMatchRuntime.waiting && battle.classList.contains('active');
...
function rankMatchHandleClick(event) {
  ...
  const rankButton = target?.closest?.('[data-rank-mode]');
  if (rankButton) return;
  const start = target?.closest?.('#startMatch');
  if (!start || rankMatchRuntime.selectedMode !== 'rank') return;
  event.preventDefault?.();
  event.stopPropagation?.();
  event.stopImmediatePropagation?.();
  enterRankMatchWaiting(rankMatchDocument());
}

function rankMatchHandleKeydown(event) {
  if (event.key === 'Escape') {
    const doc = rankMatchDocument();
    const picker = doc?.getElementById?.(RANK_MATCH_PICKER_ID);
    if (picker && !picker.hidden) {
      closeRankMatchPartnerPicker(doc);
      return;
    }
    if (rankMatchRuntime.waiting) {
      event.preventDefault?.();
      exitRankMatchWaiting(doc);
    }
  }
...
function installRankMatchPresentation() {
  if (rankMatchRuntime.installed) return true;
  const doc = rankMatchDocument();
...
if (typeof window !== 'undefined' && typeof document !== 'undefined') {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', installRankMatchPresentation, { once: true });
  } else {
    installRankMatchPresentation();
  }
}
