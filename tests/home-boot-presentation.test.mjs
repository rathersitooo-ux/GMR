...
test('Home first paint source does not generate redundant narration or dashboard chrome', () => {
  const markup = initialStaticMarkup();
  ...
  for (const className of forbiddenClasses) {
    assert.doesNotMatch(
      ...
      `${className} must not be emitted into initial Home markup; remove its producer instead of hiding it after mount`,
    );
  }
  for (const text of ['HOME VISUAL', 'GAMEROAD のホーム', 'ホームから各機能へ移動できます']) {
    assert.equal(
      ...
      false,
      `initial Home markup must not emit explanatory text: ${text}`,
    );
  }
});
