...
export const CARDS_DECK_FINDABILITY_CONTRACT = Object.freeze({
  ...
  zeroResultEscape: 'clear-current-findability-only',
  persistence: 'none',
  ownsCardData: false,
  mutatesDeck: false,
...
export function shouldOfferCardsDeckFindabilityReset({
  total = 0,
  visible = 0,
  query = '',
  deckFilter = 'all',
  ...
  const hasRestriction = Boolean(
    normalizeCardsDeckSearchQuery(query)
    || normalizedFilter !== 'all'
    || favoriteOnly === true
  );
  return Number(total) > 0 && Number(visible) === 0 && hasRestriction;
}
...
export function installCardsDeckFindability({ document: doc = globalThis.document, window: win = globalThis.window } = {}) {
  ...
  const existingSearch = doc.querySelector('#cardSearch');
  const input = existingSearch ?? doc.createElement('input');
  const ownsSearchInput = !existingSearch;
  if (ownsSearchInput) {
    input.type = 'search';
  ...
  resetButton.dataset.role = 'cards-deck-findability-reset';
  resetButton.hidden = true;
  resetButton.setAttribute?.('aria-label', '検索と絞り込みを解除');
  const count = doc.createElement('span');
  count.dataset.role = 'cards-deck-findability-count';
  count.setAttribute?.('aria-live', 'polite');
  if (ownsSearchInput) host.appendChild(input);
  ...
  const existingFilterRow = doc.querySelector('#r4SuitFilters');
  if (existingSearch && existingFilterRow) {
    host.dataset.integrated = 'true';
    existingFilterRow.appendChild(host);
  } else {
    grid.before?.(host);
  ...
  if (!doc.getElementById?.('gameroad-cards-deck-findability-style')) {
    ...
    style.textContent = '[data-role="cards-deck-findability"]{position:relative;z-index:2;display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin:6px 0 10px;padding:5px 7px;border-block:1px solid rgba(146,224,202,.18);background:linear-gradient(90deg,rgba(5,18,25,.92),rgba(8,37,32,.58) 62%,rgba(5,18,25,.18))}[data-role="cards-deck-findability"] input{min-height:44px;min-width:min(240px,58vw);padding:8px 12px;border-radius:3px;border:1px solid rgba(157,224,207,.34);background:linear-gradient(180deg,rgba(12,31,39,.94),rgba(7,20,29,.96));color:inherit;font:inherit}[data-role="cards-deck-findability"] input:focus-visible{outline:2px solid rgba(161,239,213,.72);outline-offset:2px}[data-role="cards-deck-findability"] button{min-height:44px;padding:8px 12px;border-radius:3px;border:1px solid rgba(174,232,216,.24);background:linear-gradient(135deg,rgba(24,57,63,.9),rgba(8,27,34,.95));color:inherit;font:800 12px/1 system-ui}[data-role="cards-deck-findability"] button:hover{border-color:rgba(188,244,227,.5);filter:brightness(1.08)}[data-role="cards-deck-findability"] button:focus-visible{outline:2px solid rgba(161,239,213,.78);outline-offset:1px}[data-role="cards-deck-findability"] button:active{transform:translateY(1px)}[data-role="cards-deck-findability"] button[aria-pressed="true"]{color:#fff3ba;background:linear-gradient(135deg,rgba(88,73,26,.96),rgba(47,48,27,.96));border-color:rgba(255,222,106,.76);box-shadow:inset 3px 0 0 #ffd84a}[data-role="cards-deck-findability"] button[hidden]{display:none}[data-role="cards-deck-findability-count"]{font:800 11px/1 ui-monospace,SFMono-Regular,Consolas,monospace;font-variant-numeric:tabular-nums;opacity:.78;white-space:nowrap}[data-role="cards-favorite-action"]{min-height:44px;padding:8px 12px}@media(prefers-reduced-motion:reduce){[data-role="cards-deck-findability"] button{transition:none!important;transform:none!important}}';
  ...
  const render = () => {
    ...
    if (existingSearch) {
      ...
      const nodes = [...doc.querySelectorAll('#collectionGrid [data-id]')];
      for (const node of nodes) {
        const matchesOwnFilters = matchCardsDeckFindabilityCard(
          currentCollectionFindabilityRecord(node, favoriteSet),
          { query: '', deckFilter, favoriteOnly },
        );
        if (matchesOwnFilters) delete node.dataset.cardsDeckFilterHidden;
        else node.dataset.cardsDeckFilterHidden = 'true';
      }
      const visible = nodes.filter(
        (node) => !node.hidden && node.dataset.cardsDeckFilterHidden !== 'true',
      ).length;
      ...
      result = applyCardsDeckFindability({
        document: doc,
        query: input.value,
        deckFilter,
        favoriteOnly,
    ...
    resetButton.hidden = !shouldOfferCardsDeckFindabilityReset(result);
  ...
  const onReset = () => {
    ...
    deckFilter = 'all';
    favoriteOnly = false;
    if (existingSearch && typeof win?.Event === 'function') {
  ...
  observer?.observe?.(grid, {
    ...
    subtree: true,
    attributes: true,
    attributeFilter: existingSearch
      ? ['class', 'aria-label', 'data-id', 'hidden']
      : ['class', 'aria-label', 'data-id'],
  });
  render();
  ...
  const installation = Object.freeze({
    ...
    state: () => Object.freeze({
      ...
      selectedCardId,
      zeroResultEscapeVisible: !resetButton.hidden,
    }),
    destroy() {
      if (destroyed) return;
      destroyed = true;
      ...
      for (const node of [...(doc.querySelectorAll?.('#collectionGrid [data-id]') ?? [])]) {
        if (existingSearch) delete node.dataset.cardsDeckFilterHidden;
