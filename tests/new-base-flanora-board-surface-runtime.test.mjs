...
test('mount is world-first: one top-center GOAL, twelve route anchors, four x three vertical roads, then shared field',()=>{
  ...
  assert.equal(snap.laneCount,12); assert.equal(snap.goalCount,1); assert.equal(snap.sharedGoalCount,1); assert.equal(snap.routeGateCount,12); assert.equal(snap.goalBranchCount,12); assert.equal(snap.shieldCount,12); assert.equal(snap.roadStepCount,84); assert.equal(snap.visibleBuiltRoadStepCount,0); assert.equal(snap.clearingCellCount,26);
  ...
  assert.equal(snap.visibleSharedFieldNodeCount,0); assert.equal(snap.visibleSharedFieldEdgeCount,0); assert.equal(snap.sourceClearingBindingCount,26);
  assert.match(runtime.root.getAttribute('aria-label'),/共有フィールド.*盾.*カード.*12経路ゲート.*1つのGOAL/);
  assert.deepEqual(runtime.upper.children.map(node=>node.className),['grFlanoraGoalBranches','grFlanoraSharedGoal','grFlanoraRouteGateLayer','grFlanoraProgressionGrid']);
  assert.equal(runtime.goalBranches.children.length,12);
  assert.equal(new Set(runtime.goalBranches.children.map(node=>node.getAttribute('x1'))).size,12);
  assert.equal(new Set(runtime.goalBranches.children.map(node=>node.dataset.goalBranchLaneKey)).size,12);
  assert.equal(runtime.goalBranches.children.every(node=>node.dataset.sharedGoalId==='goal:shared'),true);
  assert.equal(runtime.resolveGoalBranch('P1',0).getAttribute('x2'),'7.1419');
  ...
  assert.match(styleText,/\.grFlanoraRouteGateLayer\{[^}]*position:absolute[^}]*top:59%/);
  assert.match(styleText,/\.grFlanoraProgressionGrid\{[^}]*top:27%/);
  assert.doesNotMatch(styleText,/\.grFlanoraFieldEdge\{/);
  assert.match(styleText,/\.grFlanoraGoalBranch\{[^}]*vector-effect:non-scaling-stroke/);
  assert.doesNotMatch(styleText,/\.grFlanoraRouteGateAnchor::before\{/);
  ...
  assert.match(styleText,/\.grFlanoraRoad::before\{[^}]*opacity:\.24/);
  assert.match(styleText,/data-progression-built-count[^}]*opacity:\.46/);
  assert.match(styleText,/data-connected-to-goal="true"\][^}]*opacity:\.82/);
  ...
  assert.doesNotMatch(styleText,/clip-path:polygon/);
  assert.doesNotMatch(styleText,/#battleMap\[data-central-world-live="1"\] #fieldCanvas/);
  assert.doesNotMatch(styleText,/#battleMap\[data-central-world-live="1"\] #board\{/);
  assert.doesNotMatch(styleText,/#battleMap\[data-central-world-live="1"\] #routeSvg/);
  assert.ok(styleText.includes('@media(max-width:540px) and (orientation:portrait){'));
  assert.ok(styleText.includes('[data-new-base-flanora-board-surface="1"] .grFlanoraSharedGoal{width:70px;height:28px}'));
  assert.ok(!styleText.includes('.grFlanoraSharedGoal{top:19.5%'));
  assert.ok(!styleText.includes('.grFlanoraRouteGateLayer{top:29%'));
  assert.equal(runtime.routeGateLayer.children[0].style.left,'7.1419%');
  assert.equal(runtime.goalBranches.children[0].getAttribute('x1'),'45.2');
  assert.equal(runtime.goalBranches.children[11].getAttribute('x1'),'54.8');
  ...
  assert.equal(snap.sourceClearingBindingCount,26);
...
test('sync renders discrete nodes only for actual BUILT cards and keeps future positions invisible',()=>{
  ...
  assert.equal(runtime.resolveGoalBranch('P1',0).dataset.connectedToGoal,'false');
...
test('contract denies a second board engine and fixes one shared GOAL without fake future slots',()=>{
  ...
  assert.equal(FLANORA_BOARD_SURFACE_RUNTIME_CONTRACT.goalBranchVisual,'TWELVE_DISTINCT_LINE_ELEMENTS_NO_SHARED_TRUNK');
  ...
  assert.equal(FLANORA_BOARD_SURFACE_RUNTIME_CONTRACT.visibleSharedFieldNodeCount,0);
  assert.equal(FLANORA_BOARD_SURFACE_RUNTIME_CONTRACT.visibleSharedFieldEdgeCount,0);
  assert.equal(FLANORA_BOARD_SURFACE_RUNTIME_CONTRACT.duplicateLowerPresentationGraph,false);
  assert.equal(FLANORA_BOARD_SURFACE_RUNTIME_CONTRACT.sharedLowerFieldVisualAuthority,'EXISTING_BATTLE_ACTUAL');
});
