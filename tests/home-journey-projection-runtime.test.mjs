...
test('Home journey runtime remains noninteractive and is mounted beside existing Home sidecars', () => {
  const source = fs.readFileSync(new URL('../browser/home-boot-runtime-mount.mjs', import.meta.url), 'utf8');
  assert.match(source, /mountStudyRunFromCurrentBrowser\(\);[\s\S]*mountHomeJourneyProjection\(\);/);
