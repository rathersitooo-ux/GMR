...
test('invalid blank send does not supersede a pending valid send', async () => {
  let resolvePending;
  let seenRequest;
  const entry = createSaasunaConversationEntry({
    createSessionId: () => 'session-invalid-does-not-supersede',
    provider: {
      sendMessage(request) {
        seenRequest = request;
  ...
  const firstPromise = entry.send('有効な質問');
  const invalid = await entry.send('   ');
  assert.equal(invalid.turn.ok, false);
  assert.equal(invalid.turn.reason, 'TURN_BOUNDARY_INVALID');

  resolvePending({
    kind: 'utterance_candidate',
    partnerId: seenRequest.partnerId,
    dialogueVersion: seenRequest.dialogueVersion,
    sourceId: seenRequest.sourceId,
    text: '有効な返答',
  });
  const first = await firstPromise;
