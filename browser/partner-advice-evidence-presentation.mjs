...
const RULE_REASON = Object.freeze({
  left: '左側を優先する作戦設定から',
  right: '右側を優先する作戦設定から',
  max: '比較値が高い候補を優先する作戦設定から',
  min: '比較値が低い候補を優先する作戦設定から',
});
...
function sourceDescriptor(adviceResult, rule) {
  ...
  if (source === 'shared-legal-action-core') {
    return {
      sourceKind: 'strategy',
      badge: '作戦設定',
      why: RULE_REASON[rule] || '現在の作戦設定から',
      support: null,
      detailLines: [
        '現在の合法な候補だけから選択',
...
/**
 * Converts one already-produced advice result into a viewer-only explanation.
 * It never decides legality, changes ranking, or upgrades an observational signal into optimality.
 */
export function projectPartnerAdviceEvidencePresentation({ adviceResult, rule = null } = {}) {
  ...
  const descriptor = sourceDescriptor(adviceResult, rule);
...
[data-role="${ROOT_ROLE}"] .partnerAdviceEvidenceToggle{justify-self:start;min-height:36px;padding:6px 9px;border:1px solid rgba(185,226,214,.28);border-radius:9px;background:rgba(10,45,38,.72);color:inherit;font:850 10px/1 system-ui}
