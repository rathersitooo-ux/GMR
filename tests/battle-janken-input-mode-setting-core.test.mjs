...
const modeAuthority = Object.freeze({
  modes: BATTLE_JANKEN_INPUT_MODE,
  normalize: normalizeBattleJankenInputMode,
  projectOptions: projectBattleJankenInputModeOptions,
});
...
function createSetting(options = {}) {
  return createBattleJankenInputModeSetting({ ...options, modeAuthority });
}

test('requires the existing mode authority to be injected instead of importing runtime back', () => {
  assert.equal(BATTLE_JANKEN_INPUT_MODE_SETTING_CONTRACT.modeAuthorityInjectionRequired, true);
  assert.equal(BATTLE_JANKEN_INPUT_MODE_SETTING_CONTRACT.reusesModeAuthority, 'battle-janken-slidepad-runtime-mount.mjs');
  assert.throws(() => createBattleJankenInputModeSetting(), /authority is required/);
...
test('missing preference resolves to injected launcher and repairs writable storage', () => {
  const storage = fakeStorage();
  const result = createSetting({ storage }).read();
...
test('reads an existing valid card-pull preference unchanged', () => {
  const storage = fakeStorage({ [BATTLE_JANKEN_INPUT_MODE_SETTING_STORAGE_KEY]: BATTLE_JANKEN_INPUT_MODE.CARD_PULL });
  const result = createSetting({ storage }).read();
...
test('invalid stored preference is repaired to the injected default mode', () => {
  const storage = fakeStorage({ [BATTLE_JANKEN_INPUT_MODE_SETTING_STORAGE_KEY]: 'not-a-mode' });
  const result = createSetting({ storage }).read();
  assert.equal(result.mode, BATTLE_JANKEN_INPUT_MODE.LAUNCHER);
  assert.equal(result.source, 'DEFAULT_REPAIRED');
  assert.equal(storage.values.get(BATTLE_JANKEN_INPUT_MODE_SETTING_STORAGE_KEY), BATTLE_JANKEN_INPUT_MODE.LAUNCHER);
...
test('writes plain mode as a presentation preference only', () => {
  const storage = fakeStorage();
  const result = createSetting({ storage }).write(BATTLE_JANKEN_INPUT_MODE.PLAIN);
...
test('storage write failure falls back to memory and keeps the selected mode', () => {
  const storage = { getItem() { return null; }, setItem() { throw new Error('quota denied'); } };
  const setting = createSetting({ storage });
...
test('storage read failure fails soft to the injected default without throwing', () => {
