...
export const DECK_SWIPE_EFFECT_ASSETS = Object.freeze({
  ...
  burstSprite: Object.freeze({
    id: 'card-transfer-burst-sprite-v1',
    sourcePath: 'assets/visual/effects/card-transfer-burst-sprite-v1.png',
    runtimePath: '../assets/visual/effects/card-transfer-burst-sprite-v1.png',
    formal: true,
    readOnly: true,
    frameCount: 4,
    frameWidthPx: 272,
...
const DECK_SWIPE_BURST_SPRITE_ASSET_URL = new URL(
  DECK_SWIPE_EFFECT_ASSETS.burstSprite.runtimePath,
  import.meta.url,
).href;
const DECK_SWIPE_BURST_SPRITE_BACKGROUND_SIZE = `${DECK_SWIPE_EFFECT_ASSETS.burstSprite.frameCount * 100}% 100%`;
...
export const DEFAULT_DECK_SWIPE_PRESENTATION = Object.freeze({
  ...
  auraSizePx: 76,
  burstSpriteWidthPx: 96,
  burstSpriteHeightPx: 128,
});
...
function normalizeConfig(config = {}) {
  ...
  positive(merged.burstSpriteWidthPx, 'BURST_SPRITE_WIDTH_PX');
  positive(merged.burstSpriteHeightPx, 'BURST_SPRITE_HEIGHT_PX');
...
export function createDeckSwipeFlightPlan({ sourceRect, targetRect, reducedMotion = false, config = {} } = {}) {
  ...
  return Object.freeze({
    ...
    burstSpriteWidthPx: cfg.burstSpriteWidthPx,
    burstSpriteHeightPx: cfg.burstSpriteHeightPx,
...
function createLightTrailEffect(doc, plan, { lowPerf = false } = {}) {
  ...
  burst.className = 'gr-deck-swipe-burst';
  burst.setAttribute?.('data-role', 'deck-swipe-burst-sprite');
  burst.setAttribute?.('data-asset-id', DECK_SWIPE_EFFECT_ASSETS.burstSprite.id);
  burst.setAttribute?.('data-frame-count', String(DECK_SWIPE_EFFECT_ASSETS.burstSprite.frameCount));
  burst.setAttribute?.('aria-hidden', 'true');
  Object.assign(burst.style ?? {}, {
    left: `${plan.source.centerX}px`,
    top: `${plan.source.centerY}px`,
    width: `${plan.burstSpriteWidthPx}px`,
    height: `${plan.burstSpriteHeightPx}px`,
    backgroundImage: `url('${DECK_SWIPE_BURST_SPRITE_ASSET_URL}')`,
    backgroundSize: DECK_SWIPE_BURST_SPRITE_BACKGROUND_SIZE,
  ...
  return { layer, aura, burst, streaks, particles };
}
...
.gr-deck-swipe-burst{position:fixed;z-index:2;margin:0;pointer-events:none;transform-origin:center center;transform:translate3d(-50%,-50%,0) scale(.2);opacity:0;background-repeat:no-repeat;background-position:0% 0%;background-size:400% 100%;mix-blend-mode:screen;filter:saturate(1.45) brightness(1.24) drop-shadow(0 0 14px rgba(193,143,255,.78));will-change:transform,opacity,background-position,filter}
...
.gr-deck-swipe-low-perf .gr-deck-swipe-burst{filter:none}
...
@media (prefers-reduced-motion:reduce){.gr-deck-swipe-aura,.gr-deck-swipe-burst,.gr-deck-swipe-streak,.gr-deck-swipe-particle{display:none!important}.gr-deck-swipe-target-hit,.gr-deck-swipe-count-hit,.gr-deck-swipe-recent-add,.gr-deck-swipe-reject,.gr-deck-swipe-target-reject{animation-duration:1ms!important}}
`;
...
export function createDeckSwipePresentationController({
  ...
  function playSuccess({ sourceElement, targetElement, countElement = null, insertedElement = null, cardId = null } = {}) {
    ...
    const burstAnim = safeAnimate(flight.burst, [
      { transform: pathTransform(0, 0, 0.2), opacity: 0, backgroundPosition: '0% 0%', offset: 0 },
      { transform: pathTransform(plan.dx * 0.38, plan.dy * 0.38 + plan.arcY * 0.72, 1.02, plan.rotationDeg), opacity: 0.98, backgroundPosition: '33.333% 0%', offset: 0.32 },
      { transform: pathTransform(plan.dx * 0.72, plan.dy * 0.72 + plan.arcY * 0.3, 0.9, plan.rotationDeg * 0.55), opacity: 0.76, backgroundPosition: '66.667% 0%', offset: 0.67 },
      { transform: pathTransform(plan.dx, plan.dy, 0.58, plan.rotationDeg * 0.35), opacity: 0, backgroundPosition: '100% 0%', offset: 1 },
    ], { duration: plan.flightMs, easing: 'cubic-bezier(.18,.82,.25,1)', fill: 'forwards' });
    ...
    return Object.freeze({
      ...
      effect: Object.freeze({
        ...
        burstSpriteAssetId: DECK_SWIPE_EFFECT_ASSETS.burstSprite.id,
        burstFrameCount: DECK_SWIPE_EFFECT_ASSETS.burstSprite.frameCount,
        burstAnimation: Boolean(burstAnim),
