...
function hasForbiddenOpponentSecret(value) {
  if (!isObject(value)) return false;
  for (const [key, child] of Object.entries(value)) {
    if (OPPONENT_SECRET_KEYS.has(key)) return true;
    if (isObject(child) && hasForbiddenOpponentSecret(child)) return true;
    if (Array.isArray(child) && child.some(item => isObject(item) && hasForbiddenOpponentSecret(item))) {
      return true;
    }
  }
  return false;
...
function projectHand(hand) {
  ...
  const cards = hand.map((card, index) => {
    ...
    const presentationState = card.presentationState ?? 'focus';
...
function projectOpponent(value, index) {
  ...
  if (hasForbiddenOpponentSecret(value)) throw new TypeError('OPPONENT_SECRET_FIELD_FORBIDDEN');
  if (!nonEmptyString(value.playerId)) throw new TypeError('OPPONENT_PLAYER_ID_REQUIRED');

  const out = {
    playerId: value.playerId,
    seat: Number.isInteger(value.seat) ? value.seat : index,
    displayName: value.displayName ?? null,
    portraitRef: value.portraitRef ?? null,
    teamId: value.teamId ?? null
  };
