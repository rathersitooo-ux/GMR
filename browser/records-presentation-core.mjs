...
function normalizeViewport(viewport) {
  if (!viewport || typeof viewport !== 'object' || Array.isArray(viewport)) throw new TypeError('VIEWPORT_REQUIRED');
  const { width, height } = viewport;
  if (!Number.isFinite(width) || width <= 0 || !Number.isFinite(height) || height <= 0) throw new TypeError('VIEWPORT_INVALID');
  return { width, height };
}
...
function layoutForViewport(viewport) {
  ...
  if (landscape && width <= 900 && height <= 420) return { mode: 'short-landscape', listPercent: 42, detailPercent: 58, compactRows: true };
  if (landscape) return { mode: 'landscape', listPercent: 36, detailPercent: 64, compactRows: false };
  return { mode: 'portrait-stacked', listPercent: 100, detailPercent: 100, compactRows: true };
}

function normalizeAction(action, seen) {
  if (!action || typeof action !== 'object' || Array.isArray(action) || !nonEmptyString(action.actionId)) throw new TypeError('RECORD_ACTION_INVALID');
  if (seen.has(action.actionId)) throw new TypeError('DUPLICATE_RECORD_ACTION_ID');
  seen.add(action.actionId);
  return { actionId: action.actionId, enabled: action.enabled === true, label: optionalString(action.label), routeId: optionalString(action.routeId) };
}

function normalizeRecord(record, seenRecordIds) {
  if (!record || typeof record !== 'object' || Array.isArray(record) || !nonEmptyString(record.recordId)) throw new TypeError('RECORD_INVALID');
  ...
  const actions = record.actions == null ? [] : (() => {
  ...
  if (Object.prototype.hasOwnProperty.call(record, 'details')) normalized.details = cloneJson(record.details);
  return normalized;
}
...
function normalizeSource(input) {
  ...
  if (sourceState !== 'ready' && recordsInput.length > 0) throw new TypeError('NON_READY_SOURCE_CANNOT_HAVE_RECORDS');
  const seenRecordIds = new Set();
  return {
    state: sourceState,
    sourceId: optionalString(input.sourceId),
    message: optionalString(input.sourceMessage),
    records: recordsInput.map((record) => normalizeRecord(record, seenRecordIds))
  };
}

function resolveSelection(records, selectedRecordId) {
  if (selectedRecordId == null || selectedRecordId === '') return { selectedRecordId: null, selectedRecord: null };
...
export function createRecordsPresentation(input = {}) {
  ...
  return deepFreeze({
    ...
    accessibility: { reducedMotion: input.reducedMotion === true, lowPerf: input.lowPerf === true },
    effects: {
      motion: input.reducedMotion === true || input.lowPerf === true ? 'instant' : 'enabled',
      optionalDecoration: input.lowPerf === true ? 'minimal' : 'normal'
    },
...
export const RECORDS_PRESENTATION_CORE = Object.freeze({ schema: SCHEMA, sourceStates: SOURCE_STATES });
