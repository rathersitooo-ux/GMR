...
test('live janken selection stages before the existing card action and cancel stays local-only', async () => {
  ...
  assert.doesNotMatch(stageBody[0], /clickExistingHandCard/, 'staging must not invoke the authoritative source action');
  assert.doesNotMatch(clearBody[0], /clickExistingHandCard/, 'cancel must not invoke or roll back the authoritative source action');
...
test('live janken confirmation revalidates current source hand before exactly one existing action path', async () => {
  const { readFile } = await import('node:fs/promises');
  const source = await readFile(new URL('../browser/battle-janken-slidepad-runtime-mount.mjs', import.meta.url), 'utf8');
  ...
  assert.match(commitBody[0], /if \(!cardId\) return false;\s*const clicked = clickExistingHandCard/,
    'stale or invalid staged cards must fail closed before any source action');
});


test('live janken precommit is visibly distinct and all clear inputs converge on one semantic cancel', async () => {
  const { readFile } = await import('node:fs/promises');
  const source = await readFile(new URL('../browser/battle-janken-slidepad-runtime-mount.mjs', import.meta.url), 'utf8');
  ...
  assert.match(source, /event\?\.key !== 'Escape'[^\n]*clearStagedJankenAction\(\)/s);
  assert.match(source, /clearPrecommit: clearStagedJankenAction/);
  assert.match(source, /openForRound\(roundId\)[\s\S]*clearStagedJankenAction\(\)/);
});
