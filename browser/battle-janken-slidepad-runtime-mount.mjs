...
section[data-screen="battle"] #hand:has(.handCard[data-hand-aura-draggable="true"]:not([data-janken-reserved="true"]))::before{content:"";position:absolute;left:50%;bottom:-16px;width:clamp(170px,78%,430px);height:62px;transform:translateX(-50%);border:2px solid rgba(255,216,74,.66);border-bottom:0;border-radius:50% 50% 0 0 / 100% 100% 0 0;background:radial-gradient(ellipse at 50% 100%,rgba(255,205,55,.25),rgba(255,205,55,.09) 50%,transparent 72%);box-shadow:0 -10px 28px rgba(255,194,40,.14),inset 0 8px 22px rgba(255,229,126,.08);pointer-events:none;z-index:0}
section[data-screen="battle"] #hand .handCard[data-hand-aura-draggable="true"]:not([data-janken-reserved="true"])::after{content:"▲";position:absolute;left:50%;top:-18px;transform:translateX(-50%);color:#ffd84a;font-size:16px;line-height:1;font-weight:950;text-shadow:0 2px 4px rgba(0,0,0,.88),0 0 10px rgba(255,215,64,.72);pointer-events:none;z-index:12}
