...
test('R3 Home composition keeps the Home implementation behind a thin stable wrapper', () => {
  ...
  assert.equal(wrapper.includes('study-run-runtime-mount'), false);
  assert.equal(wrapper.includes('mountStudyRunFromCurrentBrowser'), false);
  assert.equal(base.includes('rogue-run-runtime-mount'), false);
