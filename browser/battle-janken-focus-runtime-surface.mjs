...
function cardCatalogValues(source) {
...
.grJankenLoadPanel{top:50%;bottom:auto;width:min(940px,calc(100% - 24px));transform:translate(-50%,-50%);padding:18px;box-shadow:0 0 0 100vmax rgba(6,8,12,.42),0 22px 64px rgba(0,0,0,.34)}
.grJankenLoadHero{display:grid;grid-template-columns:minmax(0,1fr) clamp(210px,30vw,360px) minmax(190px,1fr);gap:18px;align-items:center;min-height:min(68vh,540px)}
.grJankenLoadHero>.grJankenPhysicalCard{grid-column:2;width:100%;min-height:clamp(300px,42vw,520px);aspect-ratio:.68;border:3px solid #101319;border-radius:22px;padding:16px 12px 14px 54px;justify-self:center;box-shadow:0 24px 56px rgba(0,0,0,.34),0 0 0 1px rgba(255,255,255,.34) inset}
.grJankenLoadHero>.grJankenPhysicalCard .grJankenCardRank{font-size:clamp(34px,4vw,54px)}
.grJankenLoadHero>.grJankenPhysicalCard .grJankenCardName{font-size:clamp(16px,1.9vw,24px)}
.grJankenLoadHero>.grJankenPhysicalCard .grJankenCardIdentity{font-size:clamp(10px,1.15vw,14px)}
.grJankenLoadHero>.grJankenPhysicalCard .grJankenRoleBadge{font-size:clamp(12px,1.35vw,17px);padding:5px 9px}
.grJankenLoadInfo{grid-column:3;border:1px solid rgba(16,19,25,.18);border-radius:16px;background:rgba(255,255,255,.88);padding:10px;opacity:.72}
...
function renderLoad(state, { errorText, cardIndex, resolveCardArt }) {
  ...
  return `
    <section class="grJankenFocusPanel grJankenLoadPanel" data-provisional-card-hero="true" role="status" aria-label="ロード提出">
      <div class="grJankenFocusHeader">
...
function renderCommitting(state, cardIndex, resolveCardArt) {
  return `
    <section class="grJankenFocusPanel grJankenLoadPanel" data-provisional-card-hero="true" role="status" aria-label="ロード決定中">
