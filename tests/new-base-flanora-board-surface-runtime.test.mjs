...
test('mount is world-first: one top-center GOAL, twelve route anchors, four x three vertical roads, then shared field',()=>{
  ...
  assert.equal(runtime.root.dataset.clearingTrackShape,'fixed-user-target-network');
  assert.equal(snap.visibleSharedFieldNodeCount,40); assert.equal(snap.visibleSharedFieldEdgeCount,62);
  ...
  assert.match(styleText,/\.grFlanoraRouteGateLayer\{[^}]*position:absolute/);
  ...
  assert.match(styleText,/#battleMap\[data-central-world-live="1"\] #battleAdvicePartnerStage/);
  assert.match(styleText,/#battleMap\[data-central-world-live="1"\] #boardPlayers \[data-board-controlled-character\]/);
...
test('contract denies a second board engine and fixes one shared GOAL without fake future slots',()=>{
  ...
  assert.equal(FLANORA_BOARD_SURFACE_RUNTIME_CONTRACT.externalBattleChromePolicy,'BOARD_FIRST_COMPACT_NON_BOARD_VISUALS');
  assert.equal(FLANORA_BOARD_SURFACE_RUNTIME_CONTRACT.tableGridVisual,false);
  assert.equal(FLANORA_BOARD_SURFACE_RUNTIME_CONTRACT.sharedFieldVisual,'FIXED_USER_TARGET_NETWORK');
  assert.equal(FLANORA_BOARD_SURFACE_RUNTIME_CONTRACT.sharedFieldVisualTopology,'USER_FIXED_TARGET_NETWORK_R2');
  assert.equal(FLANORA_BOARD_SURFACE_RUNTIME_CONTRACT.visibleSharedFieldNodeCount,40);
  assert.equal(FLANORA_BOARD_SURFACE_RUNTIME_CONTRACT.visibleSharedFieldEdgeCount,62);
});
