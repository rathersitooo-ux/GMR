...
.gr-storage-backdrop{position:fixed;inset:0;z-index:2200;background:transparent;pointer-events:none}
.gr-storage-window{position:absolute;left:12px;top:12px;width:min(520px,calc(100vw - 24px));max-height:calc(100vh - 24px);overflow:auto;pointer-events:auto;background:#111827;color:#fff;border:1px solid rgba(255,255,255,.2);border-radius:16px;padding:14px;box-shadow:0 14px 38px rgba(0,0,0,.34)}
.gr-storage-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:10px}.gr-storage-title{font:800 18px/1.2 system-ui}.gr-storage-close{appearance:none;border:0;border-radius:10px;padding:8px 10px;cursor:pointer}
.gr-storage-columns{display:grid;grid-template-columns:1fr 1fr;gap:10px}.gr-storage-column{min-width:0;background:rgba(255,255,255,.06);border-radius:12px;padding:9px}.gr-storage-column h3{margin:0 0 8px;font:800 14px/1.2 system-ui}.gr-storage-card{display:grid;grid-template-columns:62px minmax(0,1fr);align-items:center;gap:9px;width:100%;margin:6px 0;padding:8px;border:1px solid rgba(255,255,255,.14);border-radius:10px;background:rgba(255,255,255,.08);color:#fff}.gr-storage-card-visual{display:block;width:62px;aspect-ratio:5/7;overflow:hidden;border-radius:7px;background:rgba(255,255,255,.05)}.gr-storage-card-visual>*{width:100%!important;height:100%!important;max-width:none!important;object-fit:cover!important;pointer-events:none!important}.gr-storage-card-info{display:flex;min-width:0;flex-direction:column;gap:7px}.gr-storage-card-label{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.gr-storage-card-actions{display:flex;flex-wrap:wrap;gap:6px}.gr-storage-card-actions button{cursor:pointer}
...
@media(max-width:700px){.gr-storage-window{width:min(360px,calc(100vw - 16px));left:8px;top:8px;max-height:calc(100vh - 16px);padding:11px}.gr-storage-columns{grid-template-columns:1fr;gap:8px}.gr-storage-card{grid-template-columns:54px minmax(0,1fr)}.gr-storage-card-visual{width:54px}.gr-storage-discovery-hint{left:8px;bottom:8px;padding:5px 8px;font-size:11px}}
`;
...
function finitePoint(value) { return Number.isFinite(value); }
export function shouldRevealDeckStorageFromCardsSwipe({ startX, startY, endX, endY, interactive = false, thresholdPx = 56 } = {}) {
...
function resolveCardsScreen(doc) { return doc?.querySelector?.('.screen.cards') ?? null; }
function isCardsInteractiveTarget(target) {
  try { return Boolean(target?.closest?.(CARDS_INTERACTIVE_SELECTOR)); }
  catch { return true; }
}
function byCardId(doc, cardId) {
  const id = String(cardId ?? '');
  if (!id) return null;
  return [...(doc?.querySelectorAll?.('#collectionGrid [data-id], #deckSlots [data-id], #exDeckSlots [data-id]') ?? [])].find((node) => String(node?.dataset?.id ?? '') === id) ?? null;
}
function stripCloneIdentity(node) {
  if (!node) return node;
  for (const current of [node, ...(node.querySelectorAll?.('[id], [tabindex], [role], [aria-label]') ?? [])]) {
...
function cloneStoredCardVisual(doc, cardId) {
  const source = byCardId(doc, cardId);
  if (!source) return null;
  const visualSource = source.querySelector?.('img, picture, canvas, [data-card-art], [data-card-image]') ?? source;
  return stripCloneIdentity(visualSource?.cloneNode?.(true) ?? null);
}

export function installDeckStorageCardsDiscovery({ document: doc = globalThis.document, openStorage, thresholdPx = 56 } = {}) {
  requiredFn(openStorage, 'OPEN_STORAGE');
  if (!doc?.addEventListener || !doc?.removeEventListener) return Object.freeze({ ensureHint: () => null, destroy() {} });
  let gesture = null;
  let hint = null;
  const ensureHint = () => {
  ...
  const onPointerDown = (event) => {
    const screen = resolveCardsScreen(doc);
    if (!screen?.classList?.contains?.('active') || !screen.contains?.(event?.target)) { gesture = null; return; }
    const interactive = isCardsInteractiveTarget(event?.target);
    if (interactive || !finitePoint(event?.clientX) || !finitePoint(event?.clientY)) { gesture = null; return; }
    gesture = { pointerId: event?.pointerId, startX: event.clientX, startY: event.clientY, interactive };
  };
  const finishGesture = (event, cancelled = false) => {
    ...
    const reveal = shouldRevealDeckStorageFromCardsSwipe({ ...current, endX: event?.clientX, endY: event?.clientY, thresholdPx });
...
export function mountDeckStorageCorner({ controller, buttonHost, document: doc = globalThis.document, getCardLabel = (id) => id } = {}) {
  ...
  const cardRow = (id) => {
    ...
    visual.dataset.role = 'deck-storage-card-visual';
    visual.setAttribute?.('aria-hidden', 'true');
    const clonedVisual = cloneStoredCardVisual(doc, id);
    if (clonedVisual) visual.appendChild(clonedVisual);
    const info = doc.createElement('span');
    info.className = 'gr-storage-card-info';
    const label = doc.createElement('span');
    label.className = 'gr-storage-card-label';
    ...
    info.append(label, actions);
    row.append(visual, info);
    return row;
  };
  ...
  function renderButton() {
    ...
    button.setAttribute('aria-label', view.overDeckLimit ? `メインデッキ ${view.deckCount}枚・超過分ストレージ ${view.overflowCount}枚` : `ストレージ ${view.storageCount}枚`);
  }
  function render() {
    renderButton();
    const view = controller.view();
    ...
    outsidePointerDown = (event) => {
      if (!backdrop || storageWindow?.contains?.(event?.target) || button.contains?.(event?.target)) return;
      close();
    };
