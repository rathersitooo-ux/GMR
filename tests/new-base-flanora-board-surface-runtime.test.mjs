...
test('mount is world-first: one top-center GOAL, twelve route anchors, four x three vertical roads, then shared field',()=>{
  ...
  assert.equal(runtime.root.dataset.clearingTrackShape,'organic-single-loop');
  ...
  assert.equal(runtime.resolveClearingCell('clearing:top:0').style.top,'28%');
  assert.equal(runtime.resolveClearingCell('clearing:bottom:5').style.top,'90%');
...
test('contract denies a second board engine and fixes one shared GOAL without fake future slots',()=>{
  ...
  assert.equal(FLANORA_BOARD_SURFACE_RUNTIME_CONTRACT.sharedFieldVisual,'ORGANIC_SINGLE_LOOP');
});
