...
const SUBJECT_SET = new Set(Object.values(LEARNING_SUBJECTS));
...
function normalizeExercise(raw, index) {
  ...
  const subject = code(raw.subject, `${id}.subject`);
...
export function createBrainTrainingSession({ sessionKey, exercises } = {}) {
  const key = code(sessionKey, 'sessionKey');
  const bank = normalizeBrainTrainingExerciseBank(exercises);
  return makeSession(key, bank.map(publicItem));
}
