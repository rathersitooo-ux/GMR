...
test('Friend Build defers only evidence-backed unfinished routes', () => {
  ...
  // Cards is under the active exact-current BFI repair chain, so this work unit must not
  // pre-empt that owner by hiding it before the acceptance result is terminal.
