...
test('four-lane Battle screen bridge preserves accepted attacker/defender truth without a second gameplay authority', () => {
  const rendered = [];
  ...
  const accepted = projectAcceptedBattleResolution(fourSeatResolution(1));
  const result = bridge.acceptAcceptedResolution({
    matchId: 'M-SCREEN-4',
    publicResolution: accepted
  });
  ...
  assert.equal(result.model.phase, 'attack');
  assert.equal(result.model.transition, 'ENTRY');
  assert.equal(result.model.returnIntent, 'MATCH_PLAN');
  assert.deepEqual(result.model.lanes.map(row => row.id), ['P1', 'P2', 'P3', 'P4']);
  assert.deepEqual(result.model.lanes.map(row => row.role), ['source', 'idle', 'target', 'idle']);
  assert.equal(result.model.focus.causeId, accepted.attackerId);
  assert.deepEqual(result.model.focus.targetIds, [accepted.defenderId]);
...
test('four-lane Battle screen fails closed when accepted attacker/defender do not belong to the four participants', () => {
  const bridge = createBattleScreenLivePresentationBridge({
    document: null,
    renderModel() { return true; }
  });
  bridge.begin('M-SCREEN-BAD-ACTOR');
  const input = fourSeatResolution(1);
  input.attackerId = 'UNKNOWN';
  assert.throws(
    () => bridge.acceptAcceptedResolution({
      matchId: 'M-SCREEN-BAD-ACTOR',
      publicResolution: projectAcceptedBattleResolution(input)
    }),
    /BATTLE_SCREEN_LIVE_ATTACK_IDS_INVALID/
  );
});
...
test('live adapter dispatches only committed public Battle truth into the four-lane screen bridge and stays fail-soft', () => {
  ...
  assert.equal(calls[1][1].publicResolution.attackerId, 'P1');
  assert.equal(calls[1][1].publicResolution.defenderId, 'P3');
