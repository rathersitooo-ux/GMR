...
test('double-send prevention and raw-main update path are present', () => {
  assert.match(source, /pendingComposers/);
  assert.match(source, /bypassButtons/);
  assert.match(source, /gameroadCurrentAutohookBypass/);
  assert.match(source, /raw\.githubusercontent\.com\/rathersitooo-ux\/GMR\/main\/tools\/chatgpt-current-autohook\/GAMEROAD_CURRENT_AutoHook\.user\.js/);
});
