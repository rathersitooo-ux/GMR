...
needles = [
    "source: 'browser/battle-camera-control-core.mjs'",
    "source: 'browser/battle-camera-input-router.mjs'",
    "source: 'browser/battle-camera-live-runtime.mjs'",
]
present = [needle in text for needle in needles]
if all(present):
    for needle in needles:
        if text.count(needle) != 1:
            raise SystemExit(f'camera artifact duplicate: {needle}')
    print('camera package artifacts already present exactly once')
    raise SystemExit(0)
if any(present):
    raise SystemExit(f'partial camera package artifact state: {present}')
if text.count(anchor) != 1:
    raise SystemExit(f'expected exactly one controlled-character anchor, found {text.count(anchor)}')
text = text.replace(anchor, anchor + ''.join(rows), 1)
path.write_text(text, encoding='utf-8')
for needle in needles:
    if path.read_text(encoding='utf-8').count(needle) != 1:
        raise SystemExit(f'failed to write exactly one camera artifact: {needle}')
