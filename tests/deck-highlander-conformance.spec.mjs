...
async function openScreen(page, screen) {
  const control = page.locator(`[data-go="${screen}"]:visible`).first();
  await expect(control, `visible navigation control for ${screen}`).toBeVisible();
  await control.click();
  await expect(page.locator(`section[data-screen="${screen}"]`)).toHaveClass(/active/);
}
...
test('persisted duplicate is preserved on reload, rejected by save, and cannot start a match', async ({ page }) => {
  ...
  const seeded = await page.evaluate((key) => {
    ...
    if (pack?.v !== 3 || !pack?.deck || !Array.isArray(pack.deck.main) || pack.deck.main.length !== 40) {
      throw new Error('expected current v3 40-card persisted deck');
    }
  ...
  await expect(page.locator('section[data-screen="battle"]')).not.toHaveClass(/active/);

  const fix = page.locator('#fixDeckFromSetup');
  await expect(fix).toBeVisible();
  await fix.click();
  await expect(page.locator('section[data-screen="cards"]')).toHaveClass(/active/);
