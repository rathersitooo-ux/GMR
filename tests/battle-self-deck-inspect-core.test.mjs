...
function snapshot(ids = ['SP-2', 'SP-1', 'SP-2', 'HT-1'], revision = 7) {
  return createAuthoritativeRemainingDeckSnapshot({
    matchId: 'MATCH-1',
    ownerPlayerId: 'P1',
    revision,
...
function publicProjection(value) {
  return projectRemainingDeckForViewer(value, { viewer: { id: 'SPECTATOR', authenticated: true } });
}

function ownerProjection(value) {
  return projectRemainingDeckForViewer(value, { viewer: { id: 'P1', authenticated: true } });
}
...
test('authoritative creation discards order and canonicalizes type counts', () => {
  ...
  assert.deepEqual(value.cardCounts, [
    { cardId: 'HT-1', count: 1 },
    { cardId: 'SP-1', count: 1 },
    { cardId: 'SP-2', count: 2 }
  ]);
  ...
  assert.equal(Object.isFrozen(value.cardCounts), true);
...
test('authenticated owner receives type counts while public identity and total remain stable', () => {
  const value = ownerProjection(snapshot());
...
test('opponent, spectator, and unauthenticated spoof receive public total only', () => {
  ...
  const viewers = [
    { id: 'P2', authenticated: true },
    { id: 'SPECTATOR', authenticated: true },
    { id: 'P1', authenticated: false },
    null
  ];

  for (const viewer of viewers) {
    const projected = projectRemainingDeckForViewer(value, { viewer });
...
test('shuffle and source order cannot change canonical owner counts', () => {
  ...
  assert.deepEqual(ownerProjection(first).cardCounts, ownerProjection(shuffled).cardCounts);
});

test('draw and return are represented only by new authoritative snapshots', () => {
  const before = snapshot(['A', 'A', 'B'], 10);
  const afterDraw = snapshot(['A', 'B'], 11);
  const afterReturn = snapshot(['B', 'A', 'A'], 12);

  assert.deepEqual(ownerProjection(before).cardCounts, [
    { cardId: 'A', count: 2 }, { cardId: 'B', count: 1 }
  ]);
  assert.deepEqual(ownerProjection(afterDraw).cardCounts, [
    { cardId: 'A', count: 1 }, { cardId: 'B', count: 1 }
  ]);
  assert.deepEqual(ownerProjection(afterReturn).cardCounts, [
    { cardId: 'A', count: 2 }, { cardId: 'B', count: 1 }
  ]);
  assert.deepEqual(publicProjection(afterDraw), {
    ok: true,
    status: 'ready',
...
test('empty remaining deck is valid without inventing card identities', () => {
  const value = snapshot([], 20);
  assert.deepEqual(ownerProjection(value).cardCounts, []);
  assert.equal(ownerProjection(value).typeCount, 0);
  assert.equal(publicProjection(value).total, 0);
});
...
test('tampered or order-bearing snapshots fail closed instead of leaking data', () => {
  const cases = [
    raw => { raw.total = 999; },
    raw => { raw.cardCounts[1].cardId = raw.cardCounts[0].cardId; },
    raw => { raw.cardCounts.reverse(); },
    raw => { raw.cardCounts[0].count = 0; },
    raw => { raw.remainingCardIds = ['SECRET-ORDER']; }
  ];

  for (const mutate of cases) {
    const raw = JSON.parse(JSON.stringify(snapshot()));
    mutate(raw);
    const validation = validateAuthoritativeRemainingDeckSnapshot(raw);
    assert.equal(validation.ok, false);
    const projected = projectRemainingDeckForViewer(raw, { viewer: { id: 'P1', authenticated: true } });
...
test('projection is frozen, deterministic, and does not mutate the authoritative snapshot', () => {
  ...
  const first = ownerProjection(value);
  const second = ownerProjection(value);

  assert.deepEqual(first, second);
  assert.equal(JSON.stringify(value), before);
  assert.equal(Object.isFrozen(first), true);
  assert.equal(Object.isFrozen(first.cardCounts), true);
  assert.equal(Object.isFrozen(first.cardCounts[0]), true);
});
...
test('replay composition preserves the same viewer boundary and never exposes authority-only counts', () => {
  ...
  const publicView = publicProjection(value);

  let log = createReplayLog({ matchId: 'MATCH-1', versions });
  log = appendAcceptedEvent(log, {
    kind: 'remaining_deck_snapshot',
    publicData: publicView,
    privateByViewer: {
      P1: { typeCount: owner.typeCount, cardCounts: owner.cardCounts }
    },
    authorityOnly: value
  });
  ...
  for (const replay of [opponent, spectator, spoofed]) {
    assert.equal('privateData' in replay.events[0], false);
  }
  for (const replay of [p1, opponent, spectator, spoofed]) {
