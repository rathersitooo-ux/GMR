...
function ritualDom(plan){
  ...
  const prompt=document.createElement('div');prompt.className='grPrompt';const b=document.createElement('b');b.textContent='道をタップして開く';const small=document.createElement('small');small.textContent='長押し / スペースキーに対応';prompt.append(b,small);
...
export function startGachaRitual({stage,results,reducedMotion=false,lowPerf=false,onComplete}={}){
  ...
  controller=Object.freeze({plan,crack,cancel(){done=true;cleanup();if(activeController===controller)activeController=null}});activeController=controller;return controller;
}
