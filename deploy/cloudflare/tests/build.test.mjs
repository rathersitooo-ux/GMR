...
const dependencyContract = [
  ...
  { file: 'battle-screen-presentation-core.mjs', source: 'browser/battle-screen-presentation-core.mjs', sourceArg: 'battleScreenPresentationCoreSource', expectedArg: 'expectedBattleScreenPresentationCoreBlob', artifact: 'battle_screen_presentation_core', fixture: "import './battle-conveyor-presentation-core.mjs';\nexport function createBattleScreenModel(){ return Object.freeze({}); }\nexport function auditBattleScreenModel(){ return Object.freeze({ ok: true }); }\n", currentBlob: '20ed7f337fc307c06cb1a6b67b3ec1355eb8b9f4' },
  { file: 'battle-screen-runtime-mount.mjs', source: 'browser/battle-screen-runtime-mount.mjs', sourceArg: 'battleScreenRuntimeMountSource', expectedArg: 'expectedBattleScreenRuntimeMountBlob', artifact: 'battle_screen_runtime_mount', fixture: "import './battle-screen-presentation-core.mjs';\nexport function mountBattleScreenExternalSurface(){ return Object.freeze({}); }\n", currentBlob: '43b07f7d0b8fdb8ea166e128d47810bf24b651e8' },
...
test('build fails closed on stale Browser or packaged dependency blob expectations', async () => {
  const staleCases = [['expectedBlob', /Browser blob mismatch/], ['expectedCoreBlob', /Deck save recovery core blob mismatch/], ['expectedDeckSaveAckCoreBlob', /Deck save ACK core blob mismatch/], ['expectedPresenceCoreBlob', /HATE peer presence core blob mismatch/], ['expectedNavigationCoreBlob', /Screen navigation core blob mismatch/], ['expectedReplayAdapterBlob', /Battle replay live adapter blob mismatch/], ['expectedPartnerBattleEventProjectionBlob', /Partner battle event projection blob mismatch/], ['expectedReplayCoreBlob', /Battle replay core blob mismatch/], ['expectedCardPresentationCoreBlob', /Card presentation core blob mismatch/], ['expectedBattleConveyorCoreBlob', /Battle conveyor presentation core blob mismatch/], ['expectedBattleScreenPresentationCoreBlob', /Battle screen presentation core blob mismatch/], ['expectedBattleScreenRuntimeMountBlob', /Battle screen runtime mount blob mismatch/], ['expectedBoardFacilityClassicBlob', /Board facility classic bridge blob mismatch/], ['expectedBoardFacilityCoreBlob', /Board facility core blob mismatch/], ['expectedBoardFacilityRuntimeMountBlob', /Board facility runtime mount blob mismatch/], ['expectedUiStateFeedbackCoreBlob', /UI state feedback core blob mismatch/], ['expectedUiStateFeedbackReadyPlanAdapterBlob', /UI state feedback ready-plan adapter blob mismatch/], ['expectedFieldMusicPolicyCoreBlob', /Field music policy core blob mismatch/], ['expectedPartnerAdviceRuntimeMountBlob', /Partner advice runtime mount blob mismatch/], ['expectedPartnerLegalActionAdapterBlob', /Partner legal action adapter blob mismatch/], ['expectedAdviceCollectiveEvalBlob', /Advice collective evaluator blob mismatch/]];
