...
const SOURCE_GAME_STAGE_COACHING = Object.freeze({
  'master-duel': Object.freeze({
    road: '遊戯王でいうなら、「このターンの展開の軸を決める」感覚に近い。ただし召喚でも魔法・罠でもなく、GAMEROADでは順番は固定じゃない。ロードを先に触っても移動から始めてもよく、進むと使えるロードが絞られ、ロードを触るとそのカードで進める範囲が分かる。最後に使う1枚は自分で決める',
  ...
  'duel-masters-plays': Object.freeze({
    road: 'デュエマでいうなら、今のターンに何を軸にするか決める感覚に近い。ただしマナチャージではなく、GAMEROADでは順番は固定じゃない。ロードを先に触っても移動から始めてもよく、進むと使えるロードが絞られ、ロードを触るとそのカードで進める範囲が分かる。最後に使う1枚は自分で決める',
  ...
  'pokemon-pocket': Object.freeze({
    road: 'ポケカでいうなら、今の盤面からこのターンの方針を決める感覚に近い。ただしエネルギーを付ける操作ではなく、GAMEROADでは順番は固定じゃない。ロードを先に触っても移動から始めてもよく、進むと使えるロードが絞られ、ロードを触るとそのカードで進める範囲が分かる。最後に使う1枚は自分で決める',
  ...
  shadowverse: Object.freeze({
    road: 'シャドバでいうなら、このターンの動きの軸を決める感覚に近い。ただしPPを払ってカードをプレイする操作ではなく、GAMEROADでは順番は固定じゃない。ロードを先に触っても移動から始めてもよく、進むと使えるロードが絞られ、ロードを触るとそのカードで進める範囲が分かる。最後に使う1枚は自分で決める',
...
function beginnerSuffix(focusRole) {
  if (focusRole === 'road') {
    return '順番は固定じゃない。ロードカードを先に触っても、移動から始めてもOK。進むと使えるロードカードが光って浮くし、ロードを触ればそのカードで進める範囲が分かる。最後に使う1枚は自分で選べばいい';
  }
  if (focusRole === 'battle') {
