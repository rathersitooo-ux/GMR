...
function card({ key, name, printedValue, zone = 'lane', doppel = false, currentValue = printedValue, instanceKey, ...rest }) {
  return { key, instanceKey, name, printedValue, currentValue, zone, isNormalDoppelganger: doppel, ...rest };
}

const isNormalDoppelganger = entry => entry.source.isNormalDoppelganger === true;

test('identity classification is required from the authoritative caller', () => {
  assert.throws(() => resolveDoppelgangerContinuousOnce([
    card({ key: 'd1', name: 'ドッペルゲンガー', printedValue: 5, doppel: true }),
    card({ key: 'x1', name: '別カード', printedValue: 5 }),
  ]), /DOPPELGANGER_PREDICATE_REQUIRED/);
});
...
test('same-number census uses printed/base value and ignores current/debuffed value', () => {
  const result = resolveDoppelgangerContinuousOnce([
    card({ key: 'd1', name: 'ドッペルゲンガー', printedValue: 7, currentValue: 3, doppel: true }),
...
test('duplicate projections of the same physical card are deduplicated before census', () => {
  const field = [
    card({ key: 'lane-view', instanceKey: 'physical-d1', name: 'ドッペルゲンガー', printedValue: 4, doppel: true }),
    card({ key: 'participant-view', instanceKey: 'physical-d1', name: 'ドッペルゲンガー', printedValue: 4, doppel: true }),
