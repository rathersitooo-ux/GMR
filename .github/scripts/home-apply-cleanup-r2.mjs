...
const EXPECTED_BLOB='7bff1a088b7c756edf17a3f1c7e2918db93ac390';
...
const baselineCounts={partnerNaki:count(before,'partner.naki'),selectedPartnerId:count(before,'selectedPartnerId'),playerCharacterId:count(before,'playerCharacterId'),dataGoElements:dataGoElementCount(before),navigateDetail:count(before,'navigateDetail')};

mustCount('<html lang="ja">',1,'initial html tag');
mustCount('id="homeRuntime"',1);mustCount('id="homeCharName"',1);mustCount('id="homeLivePartner"',1);mustCount('homeMount',2);mustCount("mountChar('#homeRuntime'",1);
...
replaceExact('.home.codexHome{position:absolute;inset:0;overflow:hidden;background:linear-gradient(135deg,rgb(28,34,62) 0%,rgb(18,24,38) 55%,rgb(10,14,24) 100%)!important;color:#f4f6ff}', '.home.codexHome{position:absolute;inset:0;overflow:hidden;background:linear-gradient(135deg,rgb(28,34,62) 0%,rgb(18,24,38) 55%,rgb(10,14,24) 100%)!important;color:#f4f6ff}html.grCodexHomeActive .app{background:linear-gradient(135deg,#10162b,#121827 58%,#080c16)}html.grCodexHomeActive .top{background:linear-gradient(180deg,rgba(10,14,26,.94),rgba(10,14,26,.34),transparent)}html.grCodexHomeActive .pill{border-color:rgba(180,196,240,.26);background:rgba(22,28,50,.76)}','Codex Home scoped shell override');
replaceExact('.codexHomeVisualLayer{position:absolute;left:13%;top:16%;width:74%;height:72%;overflow:hidden}', '.codexHomeVisualLayer{position:absolute;left:13%;top:16%;width:74%;height:72%;overflow:hidden;border-radius:24px;isolation:isolate;background:linear-gradient(155deg,rgba(62,76,124,.36),rgba(34,42,72,.62) 48%,rgba(12,18,34,.82));box-shadow:inset 0 0 0 1px rgba(148,168,230,.12)}','technical Home backdrop fallback');
...
const stateDeclAfter=html.match(/const state=\{[^\n]+\};/)?.[0];const rosterAfter=html.match(/const PROD_CHARS=\[[\s\S]*?\];/)?.[0];const saveLoadAfter=html.match(/function save\(\)\{[^\n]+\}\nfunction load\(\)\{[^\n]+\}/)?.[0];
if(!stateDeclAfter||!rosterAfter||!saveLoadAfter)throw Error('protected regions missing after patch');
const afterHashes={state:digest(stateDeclAfter),roster:digest(rosterAfter),saveLoad:digest(saveLoadAfter)};for(const key of Object.keys(protectedHashes))if(protectedHashes[key]!==afterHashes[key])throw Error(`protected ${key} region changed`);
const afterCounts={homeRuntime:count(html,'homeRuntime'),homeCharName:count(html,'homeCharName'),homeLivePartner:count(html,'homeLivePartner'),homeMount:count(html,'homeMount'),codexHeroRuntime:count(html,'codexHeroRuntime'),partnerNaki:count(html,'partner.naki'),selectedPartnerId:count(html,'selectedPartnerId'),playerCharacterId:count(html,'playerCharacterId'),dataGoElements:dataGoElementCount(html),navigateDetail:count(html,'navigateDetail')};
...
if(count(html,"mountChar('#homeRuntime'")!==0)throw Error('Home mount call remains');if(count(html,"document.getElementById('homeLivePartner')")!==0)throw Error('Battle still depends on Home DOM partner label');if(count(html,"publicPartner=(typeof partnerInfo==='function'?partnerInfo()?.name:'')?.trim();")!==1)throw Error('Battle selected-partner state lookup missing');if(count(html,'data-formal-background="missing"')!==1)throw Error('formal background missing marker incorrect');if(count(html,'grCodexHomeActive')<5)throw Error('Home scoped shell lifecycle/CSS not fully installed');
const homeSection=html.match(/<section\b[^>]*data-screen=["']home["'][^>]*>[\s\S]*?<\/section\s*>/i)?.[0];if(!homeSection)throw Error('Home section missing after patch');
for(const legacy of ['homeScene','homeCopy','homeLiveCard','homeRail','homeCmd','homeDock','homeMotionField','homeMotionMediaSlot','homeFoot']){const classRe=new RegExp(`class=["'][^"']*(?:^|\\s)${legacy}(?:\\s|$)`,'i');if(classRe.test(homeSection))throw Error(`legacy Home DOM class remains: ${legacy}`);}
...
const homeTargets=[...homeSection.matchAll(/data-home-target=["']([^"']+)["']/gi)].map(m=>m[1]).sort();const expectedTargets=['cards','characters','setup','shop'];if(JSON.stringify(homeTargets)!==JSON.stringify(expectedTargets))throw Error(`Home main targets changed: ${JSON.stringify(homeTargets)}`);
await writeFile(TARGET,html,'utf8');const post=await readFile(TARGET);const postBlob=spawnSync('git',['hash-object',TARGET],{encoding:'utf8'}).stdout.trim();if(postBlob===EXPECTED_BLOB)throw Error('patch produced no Browser blob change');console.log('HOME_R2_CLEANUP_PATCH_OK',JSON.stringify({preBlob:EXPECTED_BLOB,postBlob,preBytes:buf.byteLength,postBytes:post.byteLength,protectedHashes,baselineCounts,afterCounts,homeTargets}));