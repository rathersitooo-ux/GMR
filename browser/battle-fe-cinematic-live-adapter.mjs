import {
  createNakiBattleSpriteController,
...
#${SURFACE_ID} .battleFeStage .battleFeActor[data-naki-sprite="true"],#${SURFACE_ID} .battleFeStage .battleFeActor[data-naki-sprite="true"] .battleFeFigure{animation:none!important;transform:none!important;transition:none!important}
#${SURFACE_ID} .nakiBattleSprite{position:relative;width:100%;height:100%;display:grid;place-items:end center;overflow:visible;isolation:isolate}
#${SURFACE_ID} .nakiBattleSpriteImage{position:relative;z-index:1;display:block;width:100%;height:100%;object-fit:contain;object-position:50% 100%;image-rendering:auto}
#${SURFACE_ID} .nakiBattleSpriteVfx{position:absolute;z-index:2;inset:0;width:100%;height:100%;object-fit:contain;pointer-events:none;opacity:1}
#${SURFACE_ID} .nakiBattleSprite[data-vfx="none"] .nakiBattleSpriteVfx{display:none}
#${SURFACE_ID} .nakiBattleSprite[data-facing="left"] .nakiBattleSpriteImage,#${SURFACE_ID} .nakiBattleSprite[data-facing="left"] .nakiBattleSpriteVfx{transform:scaleX(-1)}
`;
...
function disposeMounts(global, mounts) {
  ...
  for (const mount of mounts) {
    ...
    if (mount?.kind === 'naki' && typeof mount.controller?.dispose === 'function') {
      try { mount.controller.dispose(); } catch {}
    }
  }
  mounts.splice(0, mounts.length);
}

function mountActorVisual(global, document, actor, stage, motion, mounts, token, seed) {
  ...
  if (characterId === 'partner.saasuna') {
    ...
    mounts.push({ kind: 'asset', node: image, actor });
    return;
  }
  if (characterId === 'partner.naki') {
    actor.actor.dataset.nakiSprite = 'true';
    const controller = createNakiBattleSpriteController({
      global,
      root: actor.host,
      role: actor.role === 'right' ? 'defender' : 'attacker',
      facing: actor.role === 'left' ? 'right' : 'left',
      seed,
      reducedMotion: motion === 'static'
    });
    mounts.push({ kind: 'naki', controller, actor });
    controller.play({
      startPhase: phaseForPresentationStage(stage, actor.role === 'right' ? 'defender' : 'attacker')
    });
    return;
  }
  const runtime = global?.GameRoadThreeCharRuntime;
  ...
  Promise.resolve(pending).then((handle) => {
    ...
    mounts.push({ kind: 'runtime', handle, actor });
...
function updateMountedVisuals(global, mounts, stage, motion) {
  const runtime = global?.GameRoadThreeCharRuntime;
  for (const mount of mounts) {
    const actor = mount?.actor;
    if (!actor) continue;
    if (mount.kind === 'asset' && mount.node) {
      mount.node.src = saasunaAssetFor(stage, actor.role);
      continue;
    }
    if (mount.kind === 'runtime' && mount.handle) {
      const state = actor.role === 'right' && stage === 'winner'
        ? 'hit'
        : (actor.role === 'left' && (stage === 'compare' || stage === 'winner') ? 'attack' : 'idle');
      try {
        runtime?.setState?.(mount.handle, state, {
          facing: actor.role === 'left' ? 'right' : 'left',
          performance: motion === 'static' ? 'low' : 'normal'
        });
      } catch {}
...
export function mountBattleFeCinematicLiveAdapter(global = globalThis, options = {}) {
  ...
  function render(snapshot = {}) {
    ...
    const eventKey = `${view.round ?? 'x'}:${view.sourceId}:${view.targetId}`;
    const motion = stageMotion(snapshot);
    if (currentView && currentEventKey === eventKey) {
      currentView = view;
      stage.dataset.stage = view.stage;
      stage.dataset.motion = motion;
      stage.dataset.eventId = eventKey;
      surface.classList.add('battleFeCinematicLive');
      surface.hidden = false;
      surface.dataset.stage = view.stage;
      const stageCopy = stage.querySelector?.('.battleFeMid strong');
      if (stageCopy) stageCopy.textContent = view.stageCopy;
      const stageLabel = stage.querySelector?.('.battleFeRound span');
      if (stageLabel) stageLabel.textContent = STAGE_LABELS[view.stage];
      const roundLabel = stage.querySelector?.('.battleFeRound b');
      if (roundLabel) roundLabel.textContent = view.round == null ? 'BATTLE' : `ROUND ${view.round}`;
      updateMountedVisuals(global, mounts, view.stage, motion);
      return view;
    }
    currentView = view;
    ...
    token += 1;
    disposeMounts(global, mounts);
    const rendered = createStage(document, { ...view, motion });
    ...
    for (const actor of rendered.actors) {
      ...
      mountActorVisual(global, document, actor, view.stage, stage.dataset.motion, mounts, String(token), eventKey);
    }
    return view;
  }
