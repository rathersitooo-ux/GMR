...
async function navigateHomeProfileRecords(page) {
  ...
  await expect.poll(() => page.evaluate(() => window.GAMEROAD_PROFILE_PRESENTATION?.recordsSnapshot?.().recordCount ?? -1)).toBeGreaterThan(0);
  return records;
}

for (const viewport of VIEWPORTS) {
  test(`recent match opens truthful inline details @ ${viewport.name}`, async ({ page }, testInfo) => {
    ...
    let row = records.locator('#recordsList .record').first();
    await expect(row, 'latest completed match renders on Records surface').toBeVisible();
    ...
    await expect(detail.locator('.recordsMatchDetailSummary')).toContainText(`二人 / ${driven.latest.rank}位`);
    await expect(detail.locator('.recordsMatchDetailSummary')).toContainText(`${driven.latest.rounds}ラウンド`);
    await expect(detail.locator('.recordsMatchDetailDeck')).toHaveText('使用デッキ：この対戦履歴では未記録です。');

    const png = await page.screenshot({ fullPage: true, animations: 'disabled' });
    await testInfo.attach(`${viewport.name}-records-selected.png`, { body: png, contentType: 'image/png' });

    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(450);
    records = await navigateHomeProfileRecords(page);
    row = records.locator('#recordsList .record').first();
    await expect(row, 'latest completed match survives reload through existing history authority').toBeVisible();
    const afterReloadText = (await row.innerText()).trim();
    expect(afterReloadText).toBe(beforeReloadText);
    await expect(records.locator('#recordsMatchDetail')).toBeHidden();

    await row.focus();
    await page.keyboard.press('Enter');
    await expect(row).toHaveAttribute('aria-expanded', 'true');
    await expect(records.locator('#recordsMatchDetail')).toBeVisible();
  });
}
