...
import { SAASUNA_ADVICE_BUSTUP_MOTION_PROFILE } from './partner-saasuna-advice-bustup-motion-profile.mjs';
...
#${ROOT_ID} .partnerAdviceBustup[data-motion-preset="quiet-idle"] img{animation:gameroadBustupQuietIdle 5.8s cubic-bezier(.4,0,.2,1) infinite}
#${ROOT_ID} .partnerAdviceBustup[data-motion-preset="quiet-guide"] img{animation:gameroadBustupQuietGuide 3.8s cubic-bezier(.4,0,.2,1) infinite}
#${ROOT_ID} .partnerAdviceBustup[data-motion-preset="active-burst"] img{animation:gameroadBustupActiveBurst .95s cubic-bezier(.18,.82,.22,1) 1}
#${ROOT_ID} .partnerAdviceBustup[data-motion-preset="quiet-active-quiet"] img{animation:gameroadBustupQuietActiveQuiet 2.45s cubic-bezier(.2,.78,.28,1) 1}
#${ROOT_ID} .partnerAdviceBustup[data-motion-preset="active-quiet"] img{animation:gameroadBustupActiveQuiet 2.8s cubic-bezier(.2,.78,.3,1) 1}
#${ROOT_ID} .partnerAdviceBustup[data-motion-preset="quiet-hold"] img{animation:gameroadBustupQuietHold 1.8s cubic-bezier(.4,0,.2,1) 1}
@keyframes gameroadBustupQuietIdle{0%,18%,100%{transform:translate3d(0,0,0) rotate(0deg)}36%{transform:translate3d(0,1.2%,0) rotate(-.25deg)}58%{transform:translate3d(0,.4%,0) rotate(.18deg)}76%{transform:translate3d(0,1%,0) rotate(0deg)}}
@keyframes gameroadBustupQuietGuide{0%,100%{transform:translate3d(0,.7%,0) rotate(-.12deg)}34%{transform:translate3d(1.2%,0,0) rotate(.35deg)}48%{transform:translate3d(2.7%,-1.4%,0) rotate(.7deg)}66%{transform:translate3d(.5%,.4%,0) rotate(0deg)}}
@keyframes gameroadBustupActiveBurst{0%{transform:translate3d(0,3%,0) scale(.97);filter:saturate(.92)}18%{transform:translate3d(-2.8%,-6%,0) scale(1.055) rotate(-2.6deg);filter:saturate(1.12)}46%{transform:translate3d(3.4%,-8%,0) scale(1.075) rotate(2.8deg)}72%{transform:translate3d(-1.3%,-3%,0) scale(1.035) rotate(-1deg)}100%{transform:translate3d(0,.7%,0) scale(1);filter:saturate(1)}}
@keyframes gameroadBustupQuietActiveQuiet{0%,12%{transform:translate3d(0,1%,0) scale(1)}24%{transform:translate3d(-2%,-5%,0) scale(1.05) rotate(-2deg)}38%{transform:translate3d(3%,-7%,0) scale(1.07) rotate(2.5deg)}54%{transform:translate3d(-1%,-2%,0) scale(1.025)}72%,100%{transform:translate3d(0,1.5%,0) scale(.995) rotate(-.2deg)}}
@keyframes gameroadBustupActiveQuiet{0%{transform:translate3d(0,2%,0) scale(.98)}14%{transform:translate3d(-3%,-6%,0) scale(1.06) rotate(-2.4deg)}34%{transform:translate3d(2%,-4%,0) scale(1.04) rotate(1.5deg)}56%{transform:translate3d(0,1%,0) scale(1)}78%,100%{transform:translate3d(0,2%,0) scale(.99) rotate(-.35deg);filter:saturate(.94)}}
@keyframes gameroadBustupQuietHold{0%{transform:translate3d(0,.2%,0)}38%{transform:translate3d(0,1.5%,0) rotate(-.3deg)}100%{transform:translate3d(0,1%,0)}}
...
function stateForTrigger(profile, triggerId) {
  const id = exactText(triggerId);
  if (!id) return null;
  const stateId = profile?.eventStateMap?.[id];
  return typeof stateId === 'string' && profile?.states?.[stateId] ? stateId : null;
}
...
export function installPartnerAdviceBustupRuntime({
  ...
  function render() {
    ...
    const isProfileActive = partnerId === profile.partnerId;
    figure.hidden = !isProfileActive;
    if (!isProfileActive) {
      clearTimer();
    ...
    if (reactionFingerprint && reactionFingerprint !== lastReactionFingerprint) {
      ...
      const stateId = stateForTrigger(profile, 'battle_card_submit');
    ...
    figure.dataset.motionPreset = status.motionPreset;
