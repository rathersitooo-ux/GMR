...
test('composes with R7 persistence callbacks without bypassing authority identity validation', async () => {
  let round = createPursuitSecretRound({
    roundId: 'durable-round',
    revision: 11,
    participantIds: ['b', 'a'],
  });
  round = commitPursuitSelection(round, {
    ...
    revision: 11,
    playerId: 'a',
    commitment: 'opaque-commitment-a',
  });

  const key = 'caller:authority:pursuit';
  const store = memoryStore();
  const adapter = createPursuitDurableSnapshotStore({ store, storageKey: key });
  await persistPursuitSecretRoundSnapshot(round, adapter.saveSnapshot);

  const serialized = store.values.get(key);
  assert.equal(typeof serialized, 'string');
  for (const forbiddenKey of ['"hand"', '"cardId"', '"value"', '"mode"', '"nonce"']) {
    assert.equal(serialized.includes(forbiddenKey), false, `durable snapshot leaked secret key: ${forbiddenKey}`);
  }
  ...
  assert.deepEqual(
    await loadPursuitSecretRoundSnapshot(adapter.loadSnapshot, expectedIdentity),
    round,
  );

  const tampered = JSON.parse(serialized);
  tampered.roundId = 'other-round';
  store.values.set(key, JSON.stringify(tampered));
  await assert.rejects(
    () => loadPursuitSecretRoundSnapshot(adapter.loadSnapshot, expectedIdentity),
    /roundId/,
  );
});
