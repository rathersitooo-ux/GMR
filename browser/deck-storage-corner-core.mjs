...
export function createStorageCornerViewModel(state, { isRoyal } = {}) {
...
export function resolveDeckEditorSwipe({ surface, deltaX, deltaY, thresholdPx = 56 } = {}) {
  ...
  if (surface === 'collection' && deltaX > 0) return Object.freeze({ action: 'deck-add' });
  if (deltaX < 0) return Object.freeze({ action: 'none', consumed: true, reason: 'user-unassigned-left-swipe' });
