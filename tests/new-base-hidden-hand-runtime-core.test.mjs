...
test('requires an explicit structurally valid hidden-hand registration payload', () => {
  assert.throws(
    ...
    /HIDDEN_HAND_MATCH_START_SNAPSHOT_REQUIRED/,
  );
  assert.throws(
    () => createNewBaseHiddenHandRuntime({}),
    /HIDDEN_HAND_MATCH_START_DECK_REQUIRED/,
  );
  assert.throws(
    () => createNewBaseHiddenHandRuntime({ deck: {} }),
    /HIDDEN_HAND_REGISTRATION_MISSING/,
  );
  assert.throws(
    () => createNewBaseHiddenHandRuntime({ deck: { hiddenHandCardIds: 'CARD-A' } }),
    /HIDDEN_HAND_REGISTRATION_INVALID/,
  );
  assert.throws(
    () => createNewBaseHiddenHandRuntime({ deck: { hiddenHandCardIds: [''] } }),
    /HIDDEN_HAND_CARD_ID_INVALID/,
  );
});

test('does not invent a hidden-hand cardinality ruling', () => {
  assert.deepEqual(
    readNewBaseHiddenHandCardIds(createNewBaseHiddenHandRuntime(matchStartSnapshot([]))),
    [],
  );
  assert.deepEqual(
    readNewBaseHiddenHandCardIds(createNewBaseHiddenHandRuntime(matchStartSnapshot(['A', 'B', 'C']))),
    ['A', 'B', 'C'],
  );
});
