...
function movieLiveState(phase = 'TITLE', overrides = {}) {
  return {
    schema: 'gameroad.battle-start-live-handoff.v1',
    presentationOnly: true,
    gameplayAuthority: false,
    gameStateWrite: false,
    loadingBlocksGameplay: false,
    generationId: 'GEN-R21',
    phase,
    reducedMotion: false,
    lowPerf: false,
    ...overrides
...
function moviePlayerProjection(mode = 'ANIMATION_PRIMARY+BOARD_WIPE', overrides = {}) {
  const layouts = {
    'BOARD_PRIMARY+ANIM_WIPE': { primarySurface: 'BOARD', wipeSurface: 'ANIMATION', wipeEnabled: true },
    'ANIMATION_PRIMARY+BOARD_WIPE': { primarySurface: 'ANIMATION', wipeSurface: 'BOARD', wipeEnabled: true },
    'BOARD_ONLY(WIPE_OFF)': { primarySurface: 'BOARD', wipeSurface: null, wipeEnabled: false }
  };
  return {
    schema: 'GAMEROAD_REPLAY_PLAYER_PRESENTATION_PROJECTION_V1',
    presentationOnly: true,
    mode,
    decisionSerial: 7,
    selectedEventId: 'EVT-R21-7',
    ...layouts[mode],
    ...overrides
  };
}

const battleMovieModule = import('../browser/battle-replay-live-adapter.mjs');

test('Battle Movie surface preserves title/entry/bridge overlays and board-safe pre-handoff semantics', async () => {
  const { projectBattleMovieSurface, BATTLE_MOVIE_SURFACE_BINDING } = await battleMovieModule;
  assert.equal(BATTLE_MOVIE_SURFACE_BINDING.authority, 'presentation_only_no_game_state_write');
  const expected = new Map([
    ['TITLE', 'BATTLE_START_TITLE'],
    ['ENTRY', 'BATTLE_START_ENTRY'],
    ['BRIDGE', 'MOVIE_READY_BRIDGE']
  ]);
  for (const [phase, overlayKind] of expected) {
    const plan = projectBattleMovieSurface({
      liveState: movieLiveState(phase),
      playerProjection: moviePlayerProjection('BOARD_PRIMARY+ANIM_WIPE'),
      viewerRole: 'player'
    });
    assert.equal(plan.overlayKind, overlayKind);
    assert.equal(plan.movieHandoff, false);
    assert.equal(plan.primarySurface, 'BOARD');
    assert.equal(plan.wipeSurface, null);
    assert.equal(plan.wipeEnabled, false);
    assert.equal(plan.gameplayAuthority, false);
    assert.equal(plan.gameStateWrite, false);
  }
});

test('Battle Movie handoff uses exact player layout while fallback/cancel always return board-safe', async () => {
  const { projectBattleMovieSurface } = await battleMovieModule;
  const handoff = projectBattleMovieSurface({
    liveState: movieLiveState('HANDOFF'),
    playerProjection: moviePlayerProjection('BOARD_PRIMARY+ANIM_WIPE'),
    viewerRole: 'player'
  });
  assert.equal(handoff.movieHandoff, true);
  assert.equal(handoff.primarySurface, 'BOARD');
  assert.equal(handoff.wipeSurface, 'ANIMATION');
  assert.equal(handoff.wipeEnabled, true);
  for (const phase of ['FALLBACK_REQUIRED', 'CANCELLED']) {
    const plan = projectBattleMovieSurface({
      liveState: movieLiveState(phase),
      playerProjection: moviePlayerProjection('ANIMATION_PRIMARY+BOARD_WIPE'),
      viewerRole: 'player'
    });
    assert.equal(plan.fallbackToBoard, true);
    assert.equal(plan.primarySurface, 'BOARD');
    assert.equal(plan.wipeSurface, null);
    assert.equal(plan.wipeEnabled, false);
  }
...
test('Battle Movie spectator accepts animation-primary only and motion restrictions stay static-only', async () => {
  const { projectBattleMovieSurface } = await battleMovieModule;
  const spectator = projectBattleMovieSurface({
    liveState: movieLiveState('HANDOFF', { reducedMotion: true }),
    playerProjection: moviePlayerProjection('ANIMATION_PRIMARY+BOARD_WIPE'),
    viewerRole: 'spectator'
  });
  assert.equal(spectator.primarySurface, 'ANIMATION');
  assert.equal(spectator.wipeSurface, 'BOARD');
  assert.equal(spectator.motion, 'static_only');
  assert.throws(() => projectBattleMovieSurface({
    liveState: movieLiveState('HANDOFF'),
    playerProjection: moviePlayerProjection('BOARD_PRIMARY+ANIM_WIPE'),
    viewerRole: 'spectator'
  }), /BATTLE_MOVIE_SPECTATOR_MODE_INVALID/);
  const lowPerf = projectBattleMovieSurface({
    liveState: movieLiveState('HANDOFF', { lowPerf: true }),
    playerProjection: moviePlayerProjection('BOARD_ONLY(WIPE_OFF)'),
    viewerRole: 'player'
  });
  assert.equal(lowPerf.motion, 'static_only');
});

test('Battle Movie surface rejects authority, layout, viewer, and secret-bearing projection violations', async () => {
  const { projectBattleMovieSurface } = await battleMovieModule;
  const base = {
    liveState: movieLiveState('HANDOFF'),
    playerProjection: moviePlayerProjection('ANIMATION_PRIMARY+BOARD_WIPE'),
  ...
  assert.throws(() => projectBattleMovieSurface({
    ...base,
    liveState: movieLiveState('HANDOFF', { gameplayAuthority: true })
  }), /BATTLE_MOVIE_LIVE_STATE_INVALID/);
  assert.throws(() => projectBattleMovieSurface({
    ...base,
    playerProjection: moviePlayerProjection('ANIMATION_PRIMARY+BOARD_WIPE', { primarySurface: 'BOARD' })
  }), /BATTLE_MOVIE_PLAYER_PROJECTION_LAYOUT_INVALID/);
  assert.throws(() => projectBattleMovieSurface({ ...base, viewerRole: 'admin' }), /BATTLE_MOVIE_VIEWER_ROLE_INVALID/);
  for (const secretKey of ['publicData', 'privateData', 'privateByViewer', 'authorityOnly', 'selectedScore', 'considered']) {
    assert.throws(() => projectBattleMovieSurface({
      ...base,
      playerProjection: moviePlayerProjection('ANIMATION_PRIMARY+BOARD_WIPE', { [secretKey]: 'NO' })
    }), /BATTLE_MOVIE_PLAYER_PROJECTION_NOT_PUBLIC_MINIMAL/);
  }
});

test('Battle Movie renderer and bridge write only presentation metadata on existing battleResolution surface', async () => {
  const { projectBattleMovieSurface, renderBattleMovieSurfacePlan, createBattleMovieSurfaceBridge } = await battleMovieModule;
  const fake = fakePresentationDocument();
  const plan = projectBattleMovieSurface({
    liveState: movieLiveState('HANDOFF'),
    playerProjection: moviePlayerProjection('ANIMATION_PRIMARY+BOARD_WIPE'),
  ...
  assert.equal(renderBattleMovieSurfacePlan(plan, { document: fake.document }), true);
  assert.equal(fake.box.dataset.battleMoviePhase, 'HANDOFF');
  assert.equal(fake.box.dataset.battleMovieViewerRole, 'spectator');
  assert.equal(fake.box.dataset.battleMovieMode, 'ANIMATION_PRIMARY+BOARD_WIPE');
  assert.equal(fake.box.dataset.battleMoviePrimary, 'ANIMATION');
  assert.equal(fake.box.dataset.battleMovieWipe, 'BOARD');
  assert.equal(fake.box.dataset.battleMovieMotion, 'allowed');
  assert.equal(fake.box.classList.contains('grBattleMovieHandoff'), true);
  assert.equal(fake.box.classList.contains('grBattleMovieFallback'), false);

  const bridge = createBattleMovieSurfaceBridge({ document: fake.document });
  const accepted = bridge.accept({
    liveState: movieLiveState('FALLBACK_REQUIRED'),
    playerProjection: moviePlayerProjection('ANIMATION_PRIMARY+BOARD_WIPE'),
    viewerRole: 'player'
  });
  assert.equal(accepted.rendered, true);
  assert.equal(accepted.plan.fallbackToBoard, true);
  assert.equal(bridge.snapshot().primarySurface, 'BOARD');
  assert.equal(fake.box.classList.contains('grBattleMovieHandoff'), false);
  assert.equal(fake.box.classList.contains('grBattleMovieFallback'), true);
});
