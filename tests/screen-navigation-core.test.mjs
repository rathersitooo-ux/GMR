...
test('temporary action SFX catalog groups common interactions and keeps navigation on its existing click', () => {
  ...
  assert.ok(Object.values(TEMPORARY_ACTION_SFX_CATALOG).every((cue) =>
    Object.isFrozen(cue) && cue.tones.every((tone) => tone.durationSec <= 0.15 && tone.gain <= 0.065)));
});
...
function fakeTemporaryAudioContext() {
  const oscillators = [];
  const gainValues = [];
  const parameter = (records = []) => ({
    setValueAtTime(value, time) { records.push({kind: 'set', value, time}); },
    linearRampToValueAtTime(value, time) { records.push({kind: 'linear', value, time}); },
    exponentialRampToValueAtTime(value, time) { records.push({kind: 'exponential', value, time}); },
  });
  return {
    oscillators, gainValues, currentTime: 2, state: 'suspended', resumeCount: 0, closeCount: 0,
    resume() { this.resumeCount += 1; return Promise.resolve(); },
    ...
    createGain() {
      const records = [];
      gainValues.push(records);
      return {gain: parameter(records), connect() {}};
    },
    destination: {},
  };
}

test('temporary SFX player honors gesture, mute and clamped SFX volume', () => {
  const audio = fakeTemporaryAudioContext();
  const player = createTemporarySfxPlayer({
    audioContextFactory: () => audio,
    readSettings: () => ({muted: false, volume: 4}),
    hasUserGesture: () => true,
  });
  assert.equal(player.play('ui_button', {isTrusted: true}), true);
  assert.equal(audio.oscillators.length, 1);
  assert.ok(Math.max(...audio.gainValues[0].map((entry) => entry.value)) <= 0.045);
  assert.equal(audio.resumeCount, 1);
  player.dispose();
  assert.equal(audio.closeCount, 1);
  const mutedAudio = fakeTemporaryAudioContext();
  const muted = createTemporarySfxPlayer({
    audioContextFactory: () => mutedAudio,
    readSettings: () => ({muted: true, volume: 1}),
    hasUserGesture: () => true,
  });
  assert.equal(muted.play('ui_button', {}), false);
  assert.equal(mutedAudio.oscillators.length, 0);
  const blockedAudio = fakeTemporaryAudioContext();
  const blocked = createTemporarySfxPlayer({
    audioContextFactory: () => blockedAudio,
    readSettings: () => ({muted: false, volume: 1}),
    hasUserGesture: () => false,
  });
  assert.equal(blocked.play('ui_button', {}), false);
  assert.equal(blockedAudio.oscillators.length, 0);
});
test('delegated SFX runtime is idempotent, plays blank taps and preserves one formal navigation sound', () => {
  ...
  const audio = fakeTemporaryAudioContext();
  const options = {documentSource: fakeDocument, audioContextFactory: () => audio, hasUserGesture: () => true};
...
test('accepted navigation click is excluded from delegated generic SFX', () => {
  ...
  const runtime = installTemporaryActionSfxRuntime({
    documentSource: fakeDocument, audioContextFactory: () => audio, hasUserGesture: () => true,
  });
