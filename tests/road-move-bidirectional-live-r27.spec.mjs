...
async function bootRoadBattle(page) {
  ...
  const fixture = await page.evaluate(() => {
    ...
    for (const card of window.__CARD_DATA__) {
      const id = card.id;
      const power = Number(card.power);
      if (![1, 3, 5, 6].includes(power)) continue;
      if (!window.roadSelectable?.(id) || window.normalMoveLimitForRoad?.(id) !== power) continue;
      if (!byPower.has(power)) byPower.set(power, id);
    }
    for (const power of [1, 3, 5, 6]) if (!byPower.has(power)) throw new Error(`missing Road fixture power ${power}`);
...
test('move-first exposes legal movement and model-derived Road candidates without touching Battle selection', async ({ page }) => {
  ...
  await page.evaluate(() => window.renderHand());
